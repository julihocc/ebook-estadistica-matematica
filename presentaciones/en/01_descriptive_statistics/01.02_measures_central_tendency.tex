...
\begin{frame}{Python Lab: Classical Measures}
  Exploratory calculation of Arithmetic Mean, Median, and Mode using NumPy and SciPy.
  \\ Script: \texttt{\footnotesize presentations/code/01\_estadistica\_descriptiva/01.02\_media\_mediana\_moda.py}
  
  \vspace{0.4em}
  {\lstset{basicstyle=\ttfamily\tiny, aboveskip=0.2em, belowskip=0.2em}
  \lstinputlisting[firstline=1,lastline=16]{../../code/01_estadistica_descriptiva/01.02_media_mediana_moda.py}
  }
\end{frame}

% Slide 9B: Live Python Lab (Pivot Calculation)
\begin{frame}{Python Lab: Pivot Calculation and Output}
  Algorithmic verification of the pivot formula ($P=9$).
  \\ Script: \texttt{\footnotesize presentations/code/01\_estadistica\_descriptiva/01.02\_media\_mediana\_moda.py}
  
  \vspace{0.2em}
  {\lstset{basicstyle=\ttfamily\tiny, aboveskip=0.2em, belowskip=0.2em}
  \lstinputlisting[firstline=18,lastline=27]{../../code/01_estadistica_descriptiva/01.02_media_mediana_moda.py}
  }
...
\begin{frame}{Course Exercises — Level 1: Fundamental}
  \begin{block}{Problem 1.2.1 (Simple / Direct)}
    Write out the individual terms of each of the following summations:
    \begin{enumerate}
...
\begin{frame}{Course Exercises — Level 2: Operational}
  \begin{block}{Problem 1.2.8 (Intermediate / Analytical Development)}
    Find the arithmetic mean of the numbers 5, 8, 11, 9, 12, 6, 14, and 10 by choosing as a \emph{pivot}:
    \begin{enumerate}
      \item $P=9$
      \item $P=20$
    \end{enumerate}
    \vspace{0.5em}
    Verify that in both cases the result matches exactly with the arithmetic mean calculated directly ($\bar{X} = 9.375$).
  \end{block}
\end{frame}

% Slide 12: Course Exercises - Level 3
\begin{frame}{Course Exercises — Level 3: Analytical}
  \begin{block}{Problem 1.2.5 (Advanced / Theoretical Connection)}
    If the deviations of $N$ numbers $X_{1}, X_{2}, \dots, X_{N}$ from a \emph{pivot} $P$ are given by $d_{i} = X_{i} - P, \; i = 1, \dots, N$ respectively, prove algebraically that:
    \begin{align}
...
\begin{frame}{Course Exercises — Level 4: Challenging}
  \begin{columns}[c]
    \begin{column}{0.48\textwidth}
      \begin{block}{Problem 1.2.9 (Expert)}
        Use the class mark of the middle interval as a pivot to calculate the mean height of the students reported in Table \ref{tab:en:1.2.1}. Verify that your result coincides exactly with the direct computation.
      \end{block}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{figure}[ht]
        ...
        \caption{Distribution of student heights}
        \label{tab:en:1.2.1}
      \end{figure}
    \end{column}
  \end{columns}
