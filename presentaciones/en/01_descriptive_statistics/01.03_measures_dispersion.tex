...
\begin{frame}{Variance and Bessel's Correction}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      {\small \color{TecAzulOscuro}\textbf{Population Variance ($\sigma^2$)}}
      \vspace{-0.1em}
      \begin{align}
    ...
    \begin{column}{0.48\textwidth}
      {\small \color{TecRojo}\textbf{Sample Variance ($S^2$)}}
      \vspace{-0.1em}
      \begin{align}
...
\begin{frame}{Python Lab: Variance and Bessel's Correction}
  ...
  \\ Script: \texttt{\footnotesize presentations/code/01\_estadistica\_descriptiva/01.03\_range\_variance\_stdev.py}
  
  \vspace{-0.1em}
  {\lstset{basicstyle=\ttfamily\tiny, aboveskip=0.2em, belowskip=0.2em}
  \lstinputlisting[firstline=4,lastline=23]{../../code/01_estadistica_descriptiva/01.03_range_variance_stdev.py}
  }
\end{frame}

% Slide 9B: Python Lab (Outliers & IQR Code)
\begin{frame}[fragile]{Python Lab: Robustness of IQR vs Deviation $S$}
  \scriptsize
  ...
  \vspace{-0.1em}
  {\lstset{basicstyle=\ttfamily\tiny, aboveskip=0.2em, belowskip=0.2em}
  \lstinputlisting[firstline=25,lastline=42]{../../code/01_estadistica_descriptiva/01.03_range_variance_stdev.py}
  }
\end{frame}

% Slide 9C: Python Lab (Verified Console Output)
\begin{frame}{Python Lab: Verified Console Output}
  Empirical results obtained by executing \texttt{01.03\_range\_variance\_stdev.py} in the terminal:
  \vspace{0.6em}

  \begin{block}{\footnotesize Console Output — Part 1: Bessel's Correction}
    {\fontsize{6.5pt}{7.5pt}\selectfont
    \texttt{--- Sample Comparison (Mean vs. Dispersion) ---} \\
    \texttt{Sample A: Mean = 50.0 ms | Std Dev (S) = 1.58 ms} \\
    \texttt{Sample B: Mean = 50.0 ms | Std Dev (S) = 31.62 ms}
    }
  \end{block}
  ...
  \begin{alertblock}{\footnotesize Console Output — Part 2: Impact of Outliers}
    {\fontsize{6.5pt}{7.5pt}\selectfont
    \texttt{--- Outlier Impact on S vs. IQR ---} \\
    \texttt{Without outlier: Std Dev S = 3.45   | IQR = 4.75} \\
    \texttt{With outlier:    Std Dev S = 133.29 | IQR = 6.00} \\
    \texttt{S Distortion: 38.6x greater       | IQR Distortion: 1.3x (virtually stable)}
    }
  \end{alertblock}
\end{frame}
