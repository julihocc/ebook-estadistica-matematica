...
% SLIDE 03: Motivation
\begin{frame}{The Third Member of the $\chi^2$--$t$--$F$ Family}
	\footnotesize
	\begin{motivacion}
		We have seen that $\chi^2$ characterizes the distribution of $S^2$, and that $t$ arises when estimating $\mu$ with unknown $\sigma$. Today we close the trio: the $F$-distribution compares \emph{two} sample variances, and generalizes this idea to compare \emph{multiple} means at once via ANOVA.
	\end{motivacion}
	\vspace{0.15cm}
	\pause
	\begin{itemize}\itemsep=0.1cm
		\item \textbf{Origin:} ratio of two independent chi-squared variables, each normalized by its degrees of freedom. \pause
		\item \textbf{Application 1:} are the variances of two populations equal? \pause
		\item \textbf{Application 2:} are the means of $k\ge 3$ groups equal? (ANOVA)
	\end{itemize}
\end{frame}
...
\begin{frame}{Modular Perspective --- The Next Challenge}
  ...
  \begin{block}{What We Have Built}
    The chi-squared, t, and F distributions form a coherent family for normal-sample inference.
  \end{block}
  \pause
  \begin{alertblock}{Next Step}
    The next chapter applies these tools to point and interval estimation.
  \end{alertblock}
