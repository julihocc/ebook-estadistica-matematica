...
\begin{frame}{From Discrete to Continuous: The Limit Theorem as a Bridge}
	\vspace{-0.15cm}
	\begin{block}{Why the Normal Distribution?}
		\footnotesize
		So far we have studied distributions for \emph{discrete} random variables taking integer values (Binomial, Poisson, Multinomial). But many phenomena in engineering and science vary continuously: measurement errors, price fluctuations, electrical noise, times between events. The Normal distribution, with its characteristic bell shape, is the universal model for these phenomena and the asymptotic limit of nearly all discrete distributions.
	\end{block}
	\vspace{-0.1cm}
	\pause
	\begin{alertblock}{Key Ideas}
		...
		\begin{itemize}\itemsep=0.06cm
			\item \textbf{Universal character:} Appears in the CLT as the limit of sums of i.i.d. variables with finite mean and variance.
			\item \textbf{Continuous model:} Density function $f(x)$ instead of probability mass $P(X=k)$; support over all of $\mathbb{R}$.
			\item \textbf{Discrete-continuous bridge:} Allows approximating the Binomial and Poisson when $n$ or $\lambda$ are large.
		\end{itemize}
	\end{alertblock}
\end{frame}
...
% SLIDE 03B: Empirical Rule 68-95-99.7
\begin{frame}{The 68-95-99.7 Empirical Rule and Symmetry Properties}
	\vspace{-0.15cm}
	\begin{block}{Empirical Rule}
		\footnotesize
		The Normal distribution has a fundamental mnemonic rule for quickly interpreting probabilities:
		\begin{itemize}\itemsep=0.06cm
	...
	\vspace{-0.1cm}
	\pause
	\begin{alertblock}{Application in Quality Control}
		\footnotesize
		$\bar{x}$ control charts use $\mu \pm 3\sigma$ limits that capture $99.73\%$ of variation under normal operation. Points outside these limits signal ``special causes'' of variability requiring investigation.
	\end{alertblock}
\end{frame}
