...
\begin{frame}{Formal Definition of the Multinomial Distribution}
\vspace{-0.15cm}
\begin{block}{Joint Probability Mass Function}
\footnotesize
For $\bm{X}=(X_{1},\ldots,X_{k})\sim\text{Mult}(n,p_{1},\ldots,p_{k})$,
\[
P(\bm{X}=\bm{x})=\frac{n!}{x_{1}!\cdots x_{k}!}
\prod_{i=1}^{k}p_{i}^{x_{i}},\qquad
\sum_{i=1}^{k}x_{i}=n,\quad \sum_{i=1}^{k}p_{i}=1.
\]
\end{block}
\begin{alertblock}{Interpretation}
\scriptsize
The coefficient counts all arrangements of the $n$ categorical outcomes; the
probability product weights each arrangement by its category frequencies.
\end{alertblock}
\end{frame}

% SLIDE 03C: Marginal Binomial Reduction
...
% SLIDE 11: Modular Perspective
\begin{frame}{Modular Perspective --- The Next Challenge}
\scriptsize
\begin{alertblock}{Next Session: Normal Approximation}
Discrete models assign mass to integer counts. The next section introduces the
Normal distribution and its continuous approximation to Binomial counts through
the De Moivre--Laplace limit.
\end{alertblock}
\vspace{0.08cm}
\begin{block}{Recommended Preparation}
Review probability density functions and the connection between fixed-category
counts and continuous approximations as $n$ grows.
\end{block}
\end{frame}
