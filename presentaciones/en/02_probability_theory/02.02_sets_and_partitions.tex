...
\begin{frame}[fragile]{Python Lab: Basic Set Operations}
  The script \texttt{02.02\_sets\_partitions.py} models event algebra using Python's native \texttt{set}:
  \vspace{0.2em}
  {\lstset{basicstyle=\fontsize{6.5pt}{7.5pt}\selectfont\ttfamily}
  \lstinputlisting[firstline=1, lastline=17]{../../code/02_teoria_probabilidad/02.02_sets_partitions.py}}
\end{frame}
...
\begin{frame}[fragile]{Python Lab: Partition Equations}
  ...
  \vspace{0.1em}
  {\lstset{basicstyle=\fontsize{6.5pt}{7.5pt}\selectfont\ttfamily}
  \lstinputlisting[firstline=19, lastline=37]{../../code/02_teoria_probabilidad/02.02_sets_partitions.py}}
\end{frame}
...
\begin{frame}[fragile]{Python Lab: Linear System Matrix}
  ...
  \vspace{0.1em}
  {\lstset{basicstyle=\fontsize{6.5pt}{7.5pt}\selectfont\ttfamily}
  \lstinputlisting[firstline=39, lastline=46]{../../code/02_teoria_probabilidad/02.02_sets_partitions.py}}
\end{frame}
...
\begin{frame}[fragile]{Python Lab: Solution and Inspection}
  ...
  \vspace{0.1em}
  {\lstset{basicstyle=\fontsize{6.5pt}{7.5pt}\selectfont\ttfamily}
  \lstinputlisting[firstline=48, lastline=54]{../../code/02_teoria_probabilidad/02.02_sets_partitions.py}}
\end{frame}
...
Event A (Even):         [2, 4, 6, 8, 10]
Event B (<= 5):         [1, 2, 3, 4, 5]
Union (A union B):      [1, 2, 3, 4, 5, 6, 8, 10]
Intersection (A & B):   [2, 4]
Difference (A \ B):     [6, 8, 10]
Complement (A'):        [1, 3, 5, 7, 9]

--- 2. Algebraic Solution via Standard Partition (4 Pieces) ---
x3 (|E & F|   - Both languages):          30 attendees
x2 (|E \ F|   - English only):            30 attendees
x1 (|F \ E|   - French only):             15 attendees
x0 (|E' & F'| - Neither language):        25 attendees
Verified partition total:                 100 attendees
    \end{verbatim}
  \end{block}
\end{frame}
