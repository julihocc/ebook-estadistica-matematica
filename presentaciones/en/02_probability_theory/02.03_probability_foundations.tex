...
\begin{frame}{Events and Subset Algebra}
  An \emph{event} $A$ is any subset of the sample space ($A \subset S$). If the observed outcome of an experiment is an element of $A$, we say that \emph{event $A$ has occurred}. \pause
...
\begin{frame}{Textbook Exercises — Tier 1: Fundamental}
  \small
  \begin{block}{Problem 2.3.1 (English Deck Sample Space)}
    Formally describe the sample space $S$ of a standard $52$-card deck considering both suits ($\spadesuit, \heartsuit, \diamondsuit, \clubsuit$) and ranks ($A, 2, \dots, 10, J, Q, K$).
  \end{block}
  
  \vspace{0.2em}
  \pause
  \begin{alertblock}{Formal Solution}
    The sample space is the Cartesian product of ranks and suits:
    $$S = \{A, 2, \dots, K\} \times \{\spadesuit, \heartsuit, \diamondsuit, \clubsuit\}$$
    Its exact cardinality is $\card{S} = 13 \times 4 = 52$ elementary, equally likely sample points ($P(e) = 1/52$).
  \end{alertblock}
\end{frame}
...
\begin{frame}{Textbook Exercises — Tier 2: Operational}
  \small
  \begin{block}{Problem 2.3.3 (Drawing Aces With and Without Replacement)}
    From a well-shuffled $52$-card deck, $2$ cards are drawn successively. Find the probability that both are aces ($A_1 \cap A_2$):
  \end{block}
  
  \vspace{0.2em}
  \pause
  \begin{enumerate}
    \item \textbf{With Replacement:} The card is returned before the second draw:
    $$P(A_1 \cap A_2) = \frac{4}{52} \times \frac{4}{52} = \frac{1}{13} \times \frac{1}{13} = \frac{1}{169} \approx 0.0059$$ \pause
    \item \textbf{Without Replacement:} The first card is not returned:
...
\begin{frame}{Textbook Exercises — Tier 3: Analytical}
  \small
  \begin{block}{Problem 2.3.7 (Axiomatic Proof of the Addition Rule)}
    Starting strictly from the three Kolmogorov axioms, prove that for any two arbitrary events $A, B \subset S$:
    $$P(A \cup B) = P(A) + P(B) - P(A \cap B)$$
  \end{block}

  \vspace{0.2em}
  \pause
  \begin{alertblock}{Disjoint Partition Proof}
    Decompose $A \cup B$ and $B$ into disjoint unions:
    \begin{align*}
    ...
    \end{align*}
    Solving and substituting yields the exact addition rule.
  \end{alertblock}
\end{frame}

% Slide 15: Textbook Exercises — Tier 4: Challenging
\begin{frame}{Textbook Exercises — Tier 4: Challenging}
  \small
  \begin{block}{Problem 2.3.9 (Boole's and Bonferroni's Bounds)}
    Prove the two fundamental structural inequalities of probability calculus:
  \end{block}

  \vspace{0.2em}
  \pause
  \begin{enumerate}
    \item \textbf{Boole's Inequality (Union Subadditivity):} The probability of a union of $n$ events never exceeds the sum of their individual probabilities:
    $$P\left( \bigcup_{i=1}^{n} A_i \right) \leq \sum_{i=1}^{n} P(A_i)$$ \pause
    \item \textbf{Bonferroni's Inequality (Intersection):} Using De Morgan's laws and subadditivity on complements yields:
