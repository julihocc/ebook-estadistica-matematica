...
\begin{frame}{In-Class Exercise --- Level 4: Challenging (2/2)}
  ...
  \pause
  \begin{alertblock}{Interpretation}
    $E[\hat{\boldsymbol\beta}]=\boldsymbol\beta+(\mathbf{X}^T\mathbf{X})^{-1}\mathbf{X}^T E[\boldsymbol\varepsilon]=\boldsymbol\beta+\mathbf{0}=\boldsymbol\beta$: the matrix OLS estimator is unbiased for any number of predictors $p$, exactly generalizing the scalar result from Section 09.03.
  \end{alertblock}
\end{frame}
