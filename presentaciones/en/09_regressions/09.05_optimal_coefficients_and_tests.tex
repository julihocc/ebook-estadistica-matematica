...
\begin{frame}{In-Class Exercise --- Operational Level 1 (1/2)}
  \footnotesize
  \begin{block}{Problem (Statement, Problem 9.8.4)}
    With $n=12$ vehicles: Age $X=\{1,2,2,3,4,5,5,6,7,8,9,10\}$ years, Price $Y=\{25,22,20,18,16,14,13,12,10,8,7,5\}$ thousand dollars. Compute $S_{XX}$, $S_{XY}$, the regression line $\hat Y=\hat\alpha+\hat\beta X$, and $R^2$.
  \end{block}

  \vspace{0.15cm}
  \pause
  \begin{alertblock}{Strategy}
    Compute $\sum X_i,\sum Y_i,\sum X_i^2,\sum Y_i^2,\sum X_iY_i$; then $S_{XX}=\sum X_i^2-(\sum X_i)^2/n$, $S_{XY}=\sum X_iY_i-(\sum X_i)(\sum Y_i)/n$.
  \end{alertblock}
\end{frame}

% Slide 11B: Exercise Level 1 (Resolution)
\begin{frame}{In-Class Exercise --- Operational Level 1 (2/2)}
  \scriptsize
  We compute the preliminary quantities and apply the OLS formulas: \pause
  ...
  \begin{alertblock}{Interpretation}
    \scriptsize
    The negative slope confirms the expected depreciation: each additional year of age reduces the estimated price by $\approx\$2.11$ thousand. $R^2\approx0.972$ indicates an excellent fit.
  \end{alertblock}
\end{frame}

% Slide 12A: Exercise Level 2 (Statement)
\begin{frame}{In-Class Exercise --- Operational Level 2 (1/2)}
  \footnotesize
  \begin{block}{Problem --- $t$-Test and CI for the Slope (Statement, Problem 9.8.6)}
    With $n=25$ batteries: $\hat Y=60.0-0.80X$, $R^2=0.720$, $S_{XX}=800.0$. Compute $SE(\hat\beta)$ and build the 95\% CI for $\beta$ ($t_{0.025,23}=2.069$).
  \end{block}

  \vspace{0.15cm}
  \pause
  \begin{alertblock}{Strategy}
    Obtain $\text{CME}$ from $\text{SSR}=R^2\cdot\text{SST}$ and $\text{SSE}=\text{SST}-\text{SSR}$ (needs $\text{SST}$; use $\text{SST}=\hat\beta^2S_{XX}/R^2$).
  \end{alertblock}
\end{frame}

% Slide 12B: Exercise Level 2 (Resolution)
\begin{frame}{In-Class Exercise --- Operational Level 2 (2/2)}
  \scriptsize
  We solve for SST from the known slope and $R^2$: \pause

  \vspace{0.05cm}
  \begin{block}{Calculation}
    \scriptsize
    $$\text{SSR}=\hat\beta^2S_{XX}=(-0.80)^2(800.0)=512.0, \qquad \text{SST}=\frac{512.0}{0.720}\approx711.1, \qquad \text{SSE}\approx199.1$$
    $$\text{CME}=\frac{199.1}{23}\approx8.657, \qquad SE(\hat\beta)=\sqrt{\frac{8.657}{800.0}}\approx0.1040$$
    $$\text{95\% CI}(\beta): -0.80\pm2.069(0.1040) = (-1.015,\,-0.585)$$
  \end{block}
  ...
  \begin{alertblock}{Interpretation}
    \scriptsize
    Since the interval does not contain zero, $\beta$ is statistically significant at the 95\% confidence level.
  \end{alertblock}
\end{frame}
