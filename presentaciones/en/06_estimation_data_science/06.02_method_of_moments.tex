...
% SLIDE 03: Motivation
\begin{frame}{From Criteria to Constructing Estimators}
	\footnotesize
	\begin{motivacion}
		In Section 06.01 we learned to \emph{evaluate} estimators (bias, efficiency, consistency). Now we learn to \emph{construct} them. The Method of Moments, proposed by Chebyshev and Pearson, is the most intuitive procedure: it equates sample moments with their population counterparts.
	\end{motivacion}
	\vspace{0.15cm}
	\pause
	\begin{itemize}\itemsep=0.1cm
		\item \textbf{Core idea:} if we know $\mu_k(\theta)$ as a function of $\theta$, and $m_k$ estimates $\mu_k$, solving $\mu_k(\theta)=m_k$ gives an estimator of $\theta$. \pause
		\item \textbf{Advantage:} simplicity, often closed-form formulas. \pause
		\item \textbf{Limitation:} generally less efficient than MLE.
	\end{itemize}
\end{frame}

% SLIDE 03B: Definition and Procedure
