...
\subtitle{Section 04.06 --- Memoryless Processes, MLE, Erlang Distribution and Queueing Systems}
...
\begin{frame}{Roadmap --- Chapter 04: Continuous Random Variables}
	...
	\begin{block}{Curricular Structure of Unit 3}
		...
		\begin{enumerate}\itemsep=0.03cm
			...
			\item \textbf{\textcolor{TecRojo}{Section 04.06: Exponential Distribution}} $\leftarrow$ \emph{Current focus}
...
% SLIDE 03: Motivation
\begin{frame}{Time Between Events: The Exponential as a Natural Model}
	\footnotesize
	\begin{motivacion}[Why the exponential distribution?]
		In Chapter 03 we modeled the \emph{number} of events in an interval (Poisson). Now we model the \emph{time between events}: if events follow a Poisson($\lambda$) process, the times between consecutive events are i.i.d. $\text{Exp}(\lambda)$. The Exponential is the most widely applied continuous distribution in reliability engineering, queueing theory, and stochastic process physics.
	\end{motivacion}
	\vspace{0.15cm}
	\pause
	\begin{itemize}\itemsep=0.1cm
		\item \textbf{Time between events:} Models waiting times until the first occurrence of a Poisson event. \pause
		\item \textbf{Memoryless:} $P(X > s + t \mid X > s) = P(X > t)$ is the defining property. \pause
		\item \textbf{Basis of Erlang and Gamma:} Sum of $n$ i.i.d. exponentials is Erlang$(n, \lambda)$, foundation of series systems.
	\end{itemize}
\end{frame}

% SLIDE 03B: Definition and Memoryless
...
% SLIDE 03C: Proof of the Memoryless Property
\begin{frame}{Memoryless Property: Proof and Uniqueness}
	\vspace{-0.15cm}
	\begin{columns}[T]
		...
		\begin{column}{0.44\textwidth}
			\begin{alertblock}{Uniqueness}
				\footnotesize
				If $g(s+t) = g(s)g(t)$ for measurable $g$, then $g(t) = e^{-ct}$. The Uniform, by contrast, does \emph{not} have this property.
			\end{alertblock}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}[fragile]{Python Lab: PDF and Memoryless Property}
	\vspace{-0.05cm}
	\scriptsize
	Validation of $\int f = 1$ and numerical demonstration of the memoryless property (\texttt{04.06\_exponential\_distribution.py}):
	\vspace{0.02cm}
