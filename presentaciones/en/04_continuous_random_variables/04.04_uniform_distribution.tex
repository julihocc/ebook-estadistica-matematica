...
% SLIDE 03: Definition and Moments
	...
	\begin{columns}[T]
		...
		\begin{column}{0.48\textwidth}
			\begin{alertblock}{Moments}
				\footnotesize
				$\E[X] = (a+b)/2$ (center of the interval). $\Var(X) = (b-a)^{2}/12$. No memory property: $P(X \ge a+b \mid X \ge a) = (b-a-b)^{+}/(b-a) \ne P(X \ge b)$ unless $a=0$.
			\end{alertblock}
		\end{column}
	\end{columns}
\end{frame}
