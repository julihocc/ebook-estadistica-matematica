...
\begin{frame}{Hoja de Ruta --- Capítulo 04: Variables Aleatorias Continuas}
  ...
  \begin{itemize}\itemsep=0.08em
    ...
    \item \textbf{04.06} Distribución Normal / Gaussiana y Puntaje $Z$ \pause
...
% Slide 14A: Ejercicio Nivel 1 (Enunciado)
\begin{frame}{Ejercicio en Clase --- Nivel 1: Fundamental (1/2)}
  \footnotesize
  \begin{block}{Problema 4.7.1 --- Distribución Gamma$(3,2)$ (Enunciado)}
    Sea $X \sim \text{Gamma}(\alpha=3, \beta=2)$. Escriba la PDF $f(x)$ e identifique sus parámetros, calcule la media $\mu_{X}=\alpha\beta$ y la varianza $\sigma_{X}^{2}=\alpha\beta^{2}$, y verifique que $\int_{0}^{\infty} f(x)\,dx = 1$ usando la definición de la función gamma.
  \end{block}
  ...
  \begin{alertblock}{Planteamiento del Modelo}
    \begin{itemize}\itemsep=0.04cm
      \item Parámetro de forma $\alpha=3$, parámetro de escala $\beta=2$.
      \item $\Gamma(3) = 2! = 2$ (caso entero de la función gamma).
      \item La normalización se verifica con el cambio de variable $u=x/2$.
    \end{itemize}
  \end{alertblock}
\end{frame}

% Slide 14B: Ejercicio Nivel 1 (Resolución)
\begin{frame}{Ejercicio en Clase --- Nivel 1: Fundamental (2/2)}
  \scriptsize
  Sustituimos los parámetros y verificamos la normalización: \pause

  \vspace{0.05cm}
  \begin{block}{Cálculos}
    \begin{itemize}\itemsep=0.05cm
      \item $f(x) = \dfrac{x^{2}e^{-x/2}}{16}$, $x>0$, con $\Gamma(3)\cdot 2^{3} = 2\cdot 8 = 16$.
      \item $\mu_{X} = 3\cdot 2 = 6$. $\sigma_{X}^{2} = 3\cdot 4 = 12$.
      \item Con $u=x/2$: $\int_{0}^{\infty} f(x)\,dx = \frac{1}{\Gamma(3)}\int_{0}^{\infty}u^{2}e^{-u}\,du = \Gamma(3)/\Gamma(3) = 1$.
    \end{itemize}
  \end{block}
  ...
  \begin{alertblock}{Interpretación}
    \scriptsize
    Con $\alpha=3$ enteros, $X$ es equivalente a la suma de 3 variables Exponenciales independientes con la misma escala $\beta=2$ (Erlang de 3 etapas), lo que explica por qué la media es tres veces la de una sola exponencial.
  \end{alertblock}
\end{frame}

% Slide 15A: Ejercicio Nivel 2 (Enunciado)
\begin{frame}{Ejercicio en Clase --- Nivel 2: Operativo (1/2)}
  \footnotesize
  \begin{block}{Problema 4.7.4 --- Distribución Beta$(3,5)$ (Enunciado)}
    Sea $X \sim \text{Beta}(\alpha=3, \beta=5)$. Escriba la PDF $f(x)$ usando la función beta $B(\alpha,\beta)$, calcule $B(3,5)$ usando su relación con la función gamma, y calcule $\E(X)$ y $\Var(X)$.
  \end{block}
  ...
  \begin{alertblock}{Aplicación}
    \scriptsize
    Este tipo de cálculo es habitual al modelar la fracción defectuosa de un lote de producción o la tasa de conversión de una campaña, donde $X$ está naturalmente acotada en $[0,1]$.
  \end{alertblock}
\end{frame}

% Slide 15B: Ejercicio Nivel 2 (Resolución)
\begin{frame}{Ejercicio en Clase --- Nivel 2: Operativo (2/2)}
  \scriptsize
  Usamos $\Gamma(3)=2$, $\Gamma(5)=24$ y $\Gamma(8)=5040$: \pause
  ...
  \begin{alertblock}{Interpretación}
    \scriptsize
    Con $\alpha < \beta$, la masa de probabilidad se concentra hacia valores bajos de $x$: se espera que solo el 37.5\% de la proporción modelada se realice, consistente con $\alpha=3$ "éxitos" frente a $\beta=5$ "fracasos" implícitos en los parámetros.
  \end{alertblock}
\end{frame}

% Slide 16A: Ejercicio Nivel 3 (Enunciado)
\begin{frame}{Ejercicio en Clase --- Nivel 3: Analítico (1/2)}
  \footnotesize
  \begin{block}{Problema 4.7.7 --- Confiabilidad Weibull (Enunciado)}
    El tiempo de vida $T$ (en años) de un componente mecánico sigue $T \sim \text{Weibull}(\beta=1.5, \eta=4)$. Escriba la PDF y la función de confiabilidad $R(t)$, calcule $P(T>3)$, y determine si la tasa de falla es creciente o decreciente.
  \end{block}
  ...
  \begin{alertblock}{Estrategia}
    \scriptsize
    La confiabilidad tiene forma cerrada $R(t) = \exp[-(t/\eta)^{\beta}]$; basta evaluar en $t=3$. El signo de $\beta - 1$ determina la monotonía de la tasa de falla $h(t)$.
  \end{alertblock}
\end{frame}

% Slide 16B: Ejercicio Nivel 3 (Resolución)
\begin{frame}{Ejercicio en Clase --- Nivel 3: Analítico (2/2)}
  \scriptsize
  Evaluamos la función de confiabilidad en $t=3$: \pause

  \vspace{0.05cm}
  \begin{block}{Demostración y Cálculo}
    \scriptsize
    \begin{align*}
      R(t) = \exp\!\left[-\left(\dfrac{t}{4}\right)^{1.5}\right], \qquad
      R(3) = \exp\!\left[-(0.75)^{1.5}\right] = e^{-0.6495} \approx 0.522.
    \end{align*}
    Como $\beta=1.5>1$, la tasa de falla $h(t)=\frac{1.5}{4}(t/4)^{0.5}$ es creciente. \quad \qedhere
  \end{block}
  ...
  \begin{alertblock}{Interpretación}
    \scriptsize
    Hay un $52.2\%$ de probabilidad de que el componente siga funcionando después de 3 años. Como $h(t)$ crece con $t$, el componente presenta \emph{desgaste}: el riesgo de falla se acelera con la edad, lo cual sugiere programar mantenimiento preventivo antes de ese punto.
  \end{alertblock}
\end{frame}

% Slide 17A: Ejercicio Nivel 4 (Enunciado)
\begin{frame}{Ejercicio en Clase --- Nivel 4: Desafiante (1/2)}
  \footnotesize
  \begin{block}{Problema 4.7.10 --- Proceso de Reemplazo Gamma/Erlang (Enunciado)}
    Un sistema reemplaza un componente crítico tan pronto falla; los tiempos entre reemplazos son i.i.d. $\text{Exp}(\lambda=0.5)$. Determine la distribución del tiempo $T$ hasta el quinto reemplazo, calcule $\E(T)$ y $\Var(T)$, y compare con un modelo $\text{Weibull}(\beta=1,\eta=2)$.
  \end{block}
  ...
  \begin{alertblock}{Estrategia}
    \scriptsize
    Use la propiedad aditiva de la Gamma con la misma escala $\beta=2=1/\lambda$, y recuerde que $\text{Weibull}(1,\eta)=\text{Exp}(1/\eta)$.
  \end{alertblock}
\end{frame}

% Slide 17B: Ejercicio Nivel 4 (Resolución)
\begin{frame}{Ejercicio en Clase --- Nivel 4: Desafiante (2/2)}
  \scriptsize
  Aplicamos la propiedad aditiva de variables Gamma independientes: \pause

  \vspace{0.05cm}
  \begin{block}{Cálculo}
    \scriptsize
    Cada $T_{i} \sim \text{Gamma}(1,2)$; por la propiedad aditiva, $T=\sum_{i=1}^{5}T_{i} \sim \text{Gamma}(5,2)$ (Erlang de 5 etapas).
    \begin{align*}
      \E(T) = 5\cdot 2 = 10 \text{ años}, \qquad \Var(T) = 5\cdot 4 = 20.
    \end{align*}
  \end{block}

  \vspace{0.05cm}
  \pause
  \begin{alertblock}{Comparación con Weibull}
    \scriptsize
    $\text{Weibull}(1,2)=\text{Exp}(0.5)$ coincide con la distribución de \emph{cada} $T_{i}$ individual, no con el tiempo acumulado $T$: la Weibull con $\beta=1$ modela un solo componente sin envejecimiento, mientras que la Gamma/Erlang modela la suma de varios tiempos de espera.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Perspectiva Modular --- Cierre del Capítulo 04}
  ...
  \begin{alertblock}{Preparación Recomendada}
    \begin{itemize}\itemsep=0.06cm
      \item Repasa el Teorema del Límite Central (Sección 04.06): es la base teórica de las distribuciones muestrales.
