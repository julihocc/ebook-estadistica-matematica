...
\begin{frame}{Esperanza Continua: $\E[X] = \int x f(x)\,dx$}
  ...
  \begin{alertblock}{Interpretación Física}
    \begin{itemize}\itemsep=0.05cm
      ...
      \item La esperanza no necesita estar en el soporte: para PDF triangular en $[0, 1]$, $\E[X] = 2/3 \in [0, 1]$ ✓.
    \end{itemize}
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 1: Fundamental (1/2)}
  \footnotesize
  \begin{block}{Problema 4.3.1 --- Esperanza y Varianza de PDF Triangular (Enunciado)}
    Sea $X$ con PDF triangular $f(x) = 2x$ para $0 \le x \le 1$. Calcule $\E[X] = \int_{0}^{1} 2x^{2}\,dx$, $\E[X^{2}] = \int_{0}^{1} 2x^{3}\,dx$, y verifique $\Var(X) = \E[X^{2}] - (\E[X])^{2}$.
  \end{block}
  ...
  \begin{alertblock}{Planteamiento del Modelo}
    \begin{itemize}\itemsep=0.04cm
      \item Soporte: $[0, 1]$ (acotado).
      \item Esperanza: $\E[X] = \int x f(x)\,dx = \int_{0}^{1} 2x^{2}\,dx$.
      \item Varianza por identidad: $\Var(X) = \E[X^{2}] - (\E[X])^{2}$.
    \end{itemize}
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 1: Fundamental (2/2)}
  \scriptsize
  Calculamos las integrales y verificamos: \pause

  \vspace{0.05cm}
  \begin{block}{Esperanza y Segundo Momento}
    \begin{align*}
  ...
  \vspace{0.05cm}
  \pause
  \begin{alertblock}{Varianza y Comparación con Mediana}
    \scriptsize
    $\Var(X) = \E[X^{2}] - (\E[X])^{2} = \dfrac{1}{2} - \dfrac{4}{9} = \dfrac{1}{18} \approx 0.0556$. La mediana es $m = 1/\sqrt{2} \approx 0.7071 > \E[X] \approx 0.6667$, diferencia debida a la asimetría de la PDF triangular.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 2: Operativo (1/2)}
  \footnotesize
  \begin{block}{Problema 4.3.5 --- Asimetría y Curtosis de Exponential (Enunciado)}
    Para $X \sim \text{Exp}(\lambda = 1)$, calcule el coeficiente de asimetría $\gamma_{1} = \E[(X - \mu)^{3}]/\sigma^{3}$ y la curtosis $\gamma_{2} = \E[(X - \mu)^{4}]/\sigma^{4} - 3$. Compare con la Normal ($\gamma_{1} = 0$, $\gamma_{2} = 0$).
  \end{block}
  ...
  \begin{alertblock}{Aplicación}
    \scriptsize
    Implemente una simulación Monte Carlo con $N = 250{,}000$ para verificar los valores teóricos. La Exponential tiene cola derecha pesada, así que $\gamma_{1} > 0$ y $\gamma_{2} > 0$.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 2: Operativo (2/2)}
  \scriptsize
  Calculamos los momentos centrales y comparamos: \pause

  \vspace{0.05cm}
  \begin{block}{Momentos Centrales de Exponential(1)}
    \begin{align*}
  ...
  \vspace{0.05cm}
  \pause
  \begin{alertblock}{Asimetría y Curtosis}
    \scriptsize
    \begin{align*}
      \gamma_{1} = \dfrac{2}{1^{3}} = 2 \quad \text{(asimetría positiva, cola derecha)}, \quad
      \gamma_{2} = \dfrac{9}{1^{4}} - 3 = 6 \quad \text{(exceso de curtosis)}.
    \end{align*}
    Comparando con la Normal ($\gamma_{1} = 0$, $\gamma_{2} = 0$), la Exponential es \emph{más asimétrica} y tiene \emph{colas más pesadas}. Monte Carlo: $\hat{\gamma}_{1} \approx 1.99$, $\hat{\gamma}_{2} \approx 5.87$ (con $N = 250{,}000$).
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 3: Analítico (1/2)}
  \footnotesize
  \begin{block}{Problema 4.3.7 --- Linealidad de la Esperanza (Enunciado)}
    Sea $X$ continua con PDF $f(x)$ y $\E[X] = \mu$. Demuestre que $\E[aX + b] = a\mu + b$ y que $\E[X + Y] = \E[X] + \E[Y]$ para constantes $a, b$ y variables aleatorias $X, Y$.
  \end{block}
  ...
  \begin{alertblock}{Estrategia de Deducción}
    \scriptsize
    Use la linealidad de la integral y el Teorema de Fubini para la suma. La esperanza es lineal incluso para variables dependientes (a diferencia de la varianza).
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 3: Analítico (2/2)}
  \scriptsize
  Demostramos la linealidad vía integral: \pause
  ...
  \begin{block}{Demostración}
    ...
    \end{align*}
    Para $\E[X + Y]$ con PDF conjunta $f_{X,Y}(x, y)$:
    \begin{align*}
      \E[X + Y] &= \int\int (x + y) f_{X,Y}(x, y)\,dx\,dy = \E[X] + \E[Y]
    \end{align*}
    por el Teorema de Fubini. La linealidad es válida \emph{incluso para variables dependientes} (a diferencia de $\Var(X+Y) = \Var(X) + \Var(Y) + 2\cov(X,Y)$). \quad \qedhere
  \end{block}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 4: Desafiante (1/2)}
  \footnotesize
  \begin{block}{Problema 4.3.9 --- Ley Total de Varianza (Enunciado)}
    Sea $X$ continua con PDF $f(x)$ y $Y$ otra variable. Demuestre la identidad de descomposición de varianza:
    $$\Var(X) = \E[\Var(X \mid Y)] + \Var(\E[X \mid Y])$$
    Esta identidad es fundamental en el análisis de varianza (ANOVA) y modelos jerárquicos.
  \end{block}
  ...
  \begin{alertblock}{Aplicación}
    \scriptsize
    En un ANOVA de un factor, la variabilidad total se descompone en \emph{variabilidad entre tratamientos} ($\Var(\E[X \mid Y])$) y \emph{variabilidad dentro de tratamientos} ($\E[\Var(X \mid Y)]$).
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 4: Desafiante (2/2)}
  \scriptsize
  Demostramos vía la ley total de esperanza: \pause

  \vspace{0.05cm}
  \begin{block}{Demostración Formal}
    \scriptsize
    Por la ley total de esperanza, $\E[X] = \E[\E[X \mid Y]]$. Expandimos:
    \begin{align*}
    ...
    \end{align*}
    El tercer término es $0$ (propiedad de torre). Los dos primeros son $\E[\Var(X \mid Y)]$ y $\Var(\E[X \mid Y])$, respectivamente. Por lo tanto: $\Var(X) = \E[\Var(X \mid Y)] + \Var(\E[X \mid Y])$. \quad \qedhere
  \end{block}
\end{frame}
