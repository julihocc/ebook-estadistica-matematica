...
\begin{frame}{Ejercicio en Clase --- Nivel 1: Fundamental (1/2)}
  \footnotesize
  \begin{block}{Problema --- IC para la Volatilidad (Enunciado)}
    Los rendimientos diarios de un índice bursátil en $n=21$ días producen $S=1.45\%$. Asumiendo normalidad, construya un IC del $90\%$ para $\sigma^2$ y para $\sigma$.
  \end{block}
  ...
  \begin{alertblock}{Planteamiento}
    \begin{itemize}\itemsep=0.04cm
      \item $\nu=20$; use $\chi^2_{20,0.05}=31.410$ y $\chi^2_{20,0.95}=10.851$.
    \end{itemize}
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 1: Fundamental (2/2)}
  \scriptsize
  Sustituimos en la fórmula del IC para $\sigma^2$: \pause

  \vspace{0.05cm}
  \begin{block}{Cálculo}
    $$(n-1)S^2 = 20(1.45)^2 = 42.05, \qquad \text{IC}_{90\%}(\sigma^2) = \left[\frac{42.05}{31.410}, \frac{42.05}{10.851}\right] = [1.339,\,3.875]\%^2$$
    $$\text{IC}_{90\%}(\sigma) = [1.157,\,1.969]\%$$
  \end{block}
  ...
  \begin{alertblock}{Interpretación}
    \scriptsize
    Con $90\%$ de confianza, la volatilidad diaria real del índice está entre $1.16\%$ y $1.97\%$ --- nótese la asimetría del intervalo, típica de $\chi^2$.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 2: Operativo (1/2)}
  \footnotesize
  \begin{block}{Problema --- Prueba A/B (Enunciado)}
    Versión A: $n_1=800$, $144$ conversiones. Versión B: $n_2=850$, $187$ conversiones. Construya un IC del $95\%$ para $p_1-p_2$ y decida si se justifica adoptar la Versión B.
  \end{block}
  ...
  \begin{alertblock}{Estrategia}
    \scriptsize
    Verifique condiciones de muestra grande, calcule $\hat p_1$, $\hat p_2$, y el error estándar de la diferencia.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 2: Operativo (2/2)}
  \scriptsize
  Calculamos ambas proporciones y su diferencia: \pause

  \vspace{0.05cm}
  \begin{block}{Cálculo}
    $$\hat p_1=0.18,\ \hat p_2=0.22,\ \hat p_1-\hat p_2=-0.04$$
    $$\text{IC}_{95\%}(p_1-p_2) = -0.04\pm1.96(0.01966) = [-0.0785,\,-0.0015]$$
  \end{block}
  ...
  \begin{alertblock}{Decisión}
    \scriptsize
    Como el intervalo completo es negativo (excluye el cero), hay evidencia estadística al $95\%$ de que la Versión B convierte mejor: se justifica adoptarla.
  \end{alertblock}
\end{frame}
