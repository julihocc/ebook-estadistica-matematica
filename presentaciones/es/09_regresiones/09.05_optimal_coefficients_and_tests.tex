...
\begin{frame}{Ejercicio en Clase --- Nivel Operativo 1 (1/2)}
  ...
  \begin{block}{Problema (Enunciado, Problema 9.8.4)}
    Con $n=12$ automóviles: Antigüedad $X=\{1,2,2,3,4,5,5,6,7,8,9,10\}$ años, Precio $Y=\{25,22,20,18,16,14,13,12,10,8,7,5\}$ miles de dólares. Calcule $S_{XX}$, $S_{XY}$, la recta de regresión $\hat Y=\hat\alpha+\hat\beta X$, y el $R^2$.
  \end{block}

  \vspace{0.15cm}
  \pause
  \begin{alertblock}{Estrategia}
    Calcule $\sum X_i,\sum Y_i,\sum X_i^2,\sum Y_i^2,\sum X_iY_i$; luego $S_{XX}=\sum X_i^2-(\sum X_i)^2/n$, $S_{XY}=\sum X_iY_i-(\sum X_i)(\sum Y_i)/n$.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel Operativo 1 (2/2)}
  ...
  \begin{block}{Cálculo}
    \scriptsize
    $$\sum X_i=62,\ \sum Y_i=170,\ \sum X_i^2=414,\ \sum Y_i^2=2836,\ \sum X_iY_i=681, \qquad \bar X\approx5.167,\ \bar Y\approx14.167$$
    $$S_{XX}=414-\frac{62^2}{12}\approx93.67, \quad S_{XY}=681-\frac{62(170)}{12}\approx-197.33$$
    $$\hat\beta=\frac{-197.33}{93.67}\approx-2.107, \qquad \hat\alpha\approx14.167-(-2.107)(5.167)\approx25.05, \qquad R^2\approx0.972$$
  \end{block}
  ...
  \begin{alertblock}{Interpretación}
    \scriptsize
    La pendiente negativa confirma la depreciación esperada: cada año adicional de antigüedad reduce el precio estimado en $\approx2.11$ mil dólares. El $R^2\approx0.972$ indica un ajuste excelente.
  \end{alertblock}
\end{frame}
