...
% Slide 12A: Ejercicio Analítico (Enunciado)
\begin{frame}{Ejercicio en Clase --- Nivel Analítico (1/2)}
  \footnotesize
  \begin{block}{Problema --- $R^2=r^2$ (Enunciado, Problema 9.8.7)}
    Con $n=30$, $S_X=4.5$, $S_Y=12.0$, $\bar X=8.0\%$, $\bar Y=15.0\%$ y $r_{XY}=0.820$. Demuestre que $R^2=r_{XY}^2$, calcule SCT/SCR/SCE, y obtenga $\hat\beta=r_{XY}(S_Y/S_X)$.
  \end{block}
  ...
  \begin{alertblock}{Estrategia}
    \scriptsize
    $\text{SCT}=(n-1)S_Y^2$; use la relación algebraica entre $r_{XY}$ y $\hat\beta$ a través de las desviaciones estándar.
  \end{alertblock}
\end{frame}

% Slide 12B: Ejercicio Analítico (Resolución)
\begin{frame}{Ejercicio en Clase --- Nivel Analítico (2/2)}
  ...
  \begin{block}{Cálculo}
    \scriptsize
    $$\text{SCT}=(29)(12.0)^2=4176.0, \qquad R^2=(0.820)^2=0.6724$$
    $$\text{SCR}=R^2\cdot\text{SCT}\approx2807.9, \qquad \text{SCE}\approx1368.1, \qquad \hat\beta=0.820(12.0/4.5)\approx2.187$$
  \end{block}
  ...
  \begin{alertblock}{Interpretación}
    \scriptsize
    La identidad $R^2=r_{XY}^2$ es exacta en regresión simple: ambos coeficientes cuantifican la misma proporción de variabilidad compartida.
  \end{alertblock}
\end{frame}

% Slide 13A: Ejercicio Desafiante 1 (Enunciado)
\begin{frame}{Ejercicio en Clase --- Nivel Desafiante 1/2 (1/2)}
  \footnotesize
  \begin{block}{Problema --- Ecuaciones Normales e Insesgo (Enunciado, Problema 9.8.9)}
    Bajo $Y_i=\alpha+\beta X_i+\varepsilon_i$ con $E[\varepsilon_i]=0$, deduzca el sistema de ecuaciones normales a partir de $\partial S/\partial\alpha=0$ y $\partial S/\partial\beta=0$, y demuestre que $\hat\alpha,\hat\beta$ son insesgados.
  \end{block}
  ...
  \begin{alertblock}{Estrategia}
    \scriptsize
    Sustituya el modelo verdadero en la fórmula cerrada de $\hat\beta$ y aplique linealidad de $E[\cdot]$.
  \end{alertblock}
\end{frame}

% Slide 13B: Ejercicio Desafiante 1 (Resolución)
\begin{frame}{Ejercicio en Clase --- Nivel Desafiante 1/2 (2/2)}
  \scriptsize
  ...
  \begin{alertblock}{Interpretación}
    \scriptsize
    Como $\hat\alpha=\bar Y-\hat\beta\bar X$ y $E[\bar Y]=\alpha+\beta\bar X$, se sigue $E[\hat\alpha]=\alpha$: ambos estimadores MCO son insesgados sin asumir normalidad.
  \end{alertblock}
\end{frame}

% Slide 14A: Ejercicio Desafiante 2 (Enunciado)
\begin{frame}{Ejercicio en Clase --- Nivel Desafiante 2/2 (1/2)}
  \footnotesize
  \begin{block}{Problema --- Varianzas y Ortogonalidad de Residuos (Enunciado, Problema 9.8.10)}
    Con $c_i=(X_i-\bar X)/S_{XX}$, demuestre $\sum c_i=0$, $\sum c_iX_i=1$, $\sum c_i^2=1/S_{XX}$; deduzca $\text{Var}(\hat\beta)=\sigma^2/S_{XX}$; y pruebe las 4 propiedades de ortogonalidad de los residuos MCO.
  \end{block}
  ...
  \begin{alertblock}{Estrategia}
    \scriptsize
    Aplique las leyes de varianza de sumas independientes sobre la representación lineal $\hat\beta=\sum c_iY_i$.
  \end{alertblock}
\end{frame}

% Slide 14B: Ejercicio Desafiante 2 (Resolución)
\begin{frame}{Ejercicio en Clase --- Nivel Desafiante 2/2 (2/2)}
  \scriptsize
  Aplicamos varianza a la combinación lineal de observaciones independientes: \pause

  \vspace{0.05cm}
  \begin{block}{Cálculo}
    \scriptsize
    $$\text{Var}(\hat\beta)=\text{Var}\left(\sum c_iY_i\right)=\sigma^2\sum c_i^2=\frac{\sigma^2}{S_{XX}}$$
    Las 4 propiedades de los residuos $\hat\varepsilon_i=Y_i-\hat Y_i$: (a) $\sum\hat\varepsilon_i=0$; (b) $\sum X_i\hat\varepsilon_i=0$; (c) $\sum\hat Y_i\hat\varepsilon_i=0$; (d) $\bar Y=\hat\alpha+\hat\beta\bar X$.
  \end{block}
  ...
  \begin{alertblock}{Interpretación}
    \scriptsize
    Estas cuatro identidades son la razón geométrica exacta detrás de la descomposición SCT=SCR+SCE demostrada en esta sección.
  \end{alertblock}
\end{frame}
