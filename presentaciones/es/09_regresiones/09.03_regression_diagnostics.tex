...
\title[Diagnóstico de Regresión]{Sección 09.03: Diagnóstico de Residuos y Multicolinealidad}
...
\begin{frame}{Hoja de Ruta --- Capítulo 09: Regresiones Lineales y Múltiples}
  ...
  \begin{itemize}\itemsep=0.08em
    ...
    \item \textbf{09.03} Diagnóstico de Residuos, Multicolinealidad (VIF) y Supuestos \emph{(Hoy)} \pause
...
\begin{frame}{¿Es Confiable el Modelo que Ajustamos?}
  \footnotesize
  \begin{motivacion}
    En las Secciones 09.01 y 09.02 construimos y resolvimos el modelo de regresión, pero toda inferencia (intervalos, pruebas $t$/$F$) descansa en supuestos sobre el término de error $\varepsilon$ que \textbf{nunca se verifican automáticamente}. Un modelo con $R^2$ alto puede ser completamente inválido si sus residuos violan estos supuestos.
  \end{motivacion}
...
\begin{frame}{Multicolinealidad: el Factor de Inflación de Varianza}
  ...
  \begin{alertblock}{Identidad con la Matriz de Correlación}
    $\text{VIF}_j=(\mathbf{R}^{-1})_{jj}$: un VIF alto es la manifestación diagnóstica de que $\mathbf{X}^T\mathbf{X}$ se aproxima a la singularidad (Sección 09.02). Umbral convencional: $\text{VIF}>5$ amerita Ridge, Lasso, o eliminación de variables.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 1: Fundamental (1/2)}
  \footnotesize
  \begin{block}{Problema (Enunciado, Problema 9.14.1)}
    Con $n=10$ residuos ordenados temporalmente: $e=\{2.1,1.8,-0.5,-1.2,-2.0,-1.5,0.8,1.9,2.2,1.0\}$. Calcule el estadístico de Durbin-Watson y determine el tipo de autocorrelación presente.
  \end{block}

  \vspace{0.15cm}
  \pause
  \begin{alertblock}{Planteamiento}
    $DW=\sum(e_i-e_{i-1})^2/\sum e_i^2$; compare contra el valor de referencia $DW=2$.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 1: Fundamental (2/2)}
  \scriptsize
  Calculamos numerador y denominador por separado: \pause
  ...
  \begin{alertblock}{Interpretación}
    Como $DW\approx0.576\ll2$, existe evidencia fuerte de \textbf{autocorrelación positiva}: observaciones consecutivas tienden a compartir signo, violando la independencia de los errores.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 2: Operativo (1/2)}
  \footnotesize
  \begin{block}{Problema --- VIF (Enunciado, Problema 9.14.5)}
    Con $p=3$ predictores financieros: $R_1^2=0.85$, $R_2^2=0.55$, $R_3^2=0.30$. Calcule el VIF de cada uno, decida cuáles superan el umbral $\text{VIF}>5$, y proponga un remedio de la Sección 09.02.
  \end{block}

  \vspace{0.15cm}
  \pause
  \begin{alertblock}{Estrategia}
    $\text{VIF}_j=1/(1-R_j^2)$ para cada predictor.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 2: Operativo (2/2)}
  \scriptsize
  Calculamos los tres VIF directamente: \pause
  ...
  \pause
  \begin{alertblock}{Interpretación}
    Solo el predictor 1 supera el umbral ($6.667>5$): multicolinealidad severa. Dado que no se desea eliminarlo, la \textbf{regresión Ridge} es el remedio más adecuado, pues estabiliza $(\mathbf{X}^T\mathbf{X}+\lambda\mathbf{I})^{-1}$ sin descartar la variable.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Cierre de Sección: Diagnóstico de Regresión}
  ...
  \begin{block}{Lecciones Clave}
    \begin{itemize}\itemsep=0.08cm
      ...
      \item \textbf{VIF$_j=(\mathbf{R}^{-1})_{jj}$:} un VIF alto refleja la misma inestabilidad numérica de $\mathbf{X}^T\mathbf{X}$ vista en la Sección 09.02.
...
\begin{frame}{Perspectiva Modular --- El Próximo Reto}
  \scriptsize
  \begin{retoclase}
    Con el modelo auditado internamente, la pregunta final del capítulo es la más importante en la práctica: ¿generaliza el modelo a datos \textbf{nuevos}, nunca vistos durante el ajuste? La Sección 09.04 (cierre del capítulo) presenta la validación cruzada $k$-fold, las métricas de error extra-muestral, y la implementación profesional con \texttt{scikit-learn}.
  \end{retoclase}
  ...
  \begin{alertblock}{Preparación Recomendada}
    \begin{itemize}\itemsep=0.06cm
      \item Reflexiona sobre por qué un modelo puede pasar todos los diagnósticos de esta sección y aun así generalizar mal (sobreajuste).
      \item Repasa la diferencia conceptual entre error dentro de la muestra y error fuera de la muestra.
    \end{itemize}
  \end{alertblock}
\end{frame}
