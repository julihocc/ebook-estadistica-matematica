...
\begin{frame}{Ejercicio en Clase --- Nivel Fundamental (1/2)}
  \footnotesize
  \begin{block}{Problema (Enunciado, Problema 9.20.1)}
    Un diagrama de dispersión \texttt{hp} vs. \texttt{mpg} muestra una curva descendente que se aplana y repunta ligeramente (forma de U invertida asimétrica). Identifique la transformación/especificación apropiada y explique por qué un modelo lineal sería inadecuado.
  \end{block}

  \vspace{0.15cm}
  \pause
  \begin{alertblock}{Estrategia}
    Un modelo lineal solo captura tendencias monótonas; considere agregar un término $X^2$.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel Fundamental (2/2)}
  \scriptsize
  Analizamos la forma geométrica descrita: \pause

  \vspace{0.05cm}
  \begin{block}{Diagnóstico}
    El patrón no monótono (cambio de dirección) es característico de una relación \textbf{cuadrática}. La especificación adecuada es $\texttt{mpg}=c_0+c_1\,\texttt{hp}+c_2\,\texttt{hp}^2$.
  \end{block}

  \vspace{0.05cm}
  \pause
  \begin{alertblock}{Interpretación}
    Un modelo lineal fuerza una tendencia monótona sobre datos que cambian de dirección, produciendo residuos sistemáticamente sesgados y subestimando drásticamente la calidad real del ajuste alcanzable.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel Operativo (1/2)}
  \footnotesize
  \begin{block}{Problema --- Ajuste Cuadrático Manual (Enunciado, Problema 9.20.4)}
    Con $\texttt{hp}=\{50,75,100,125,150\}$, $\texttt{mpg}=\{35,28,24,26,32\}$, ajuste $\texttt{mpg}=c_0+c_1\,\texttt{hp}+c_2\,\texttt{hp}^2$ construyendo $\mathbf{X}=[\mathbf{1},\texttt{hp},\texttt{hp}^2]$ y resolviendo la Ecuación Normal.
  \end{block}

  \vspace{0.1cm}
  \pause
  \begin{alertblock}{Estrategia}
    $\hat{\mathbf{c}}=(\mathbf{X}^T\mathbf{X})^{-1}\mathbf{X}^T\mathbf{Y}$, exactamente como en la Sección 09.07.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel Operativo (2/2)}
  \scriptsize
  Resolvemos el sistema $3\times3$ de la Ecuación Normal: \pause
  ...
  \pause
  \begin{alertblock}{Interpretación}
    El vértice de la parábola ($\texttt{hp}^*=-c_1/(2c_2)\approx104.4$) marca el punto de \texttt{mpg} mínimo predicho por el modelo.
  \end{alertblock}
\end{frame}
