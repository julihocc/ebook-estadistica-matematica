...
\begin{frame}{Ejercicio en Clase --- Nivel 1: Fundamental (1/2)}
  \footnotesize
  \begin{block}{Problema 5.1.1 --- Media y Varianza Muestral (Enunciado)}
    Una muestra aleatoria simple de tamaño $n=5$ produce las observaciones $\{8, 12, 10, 14, 6\}$. Calcule la media muestral $\bar{X}$ y la varianza muestral insesgada $S^2$.
  \end{block}
...
\begin{frame}{Ejercicio en Clase --- Nivel 1: Fundamental (2/2)}
  \scriptsize
  Calculamos media y desviaciones al cuadrado: \pause

  \vspace{0.05cm}
  \begin{block}{Cálculos}
    \begin{itemize}\itemsep=0.05cm
      \item $\bar{X} = (8+12+10+14+6)/5 = 50/5 = 10$.
      \item Desviaciones al cuadrado: $4, 4, 0, 16, 16$; suma $= 40$.
      \item $S^2 = 40/(5-1) = 10$.
    \end{itemize}
  \end{block}
  ...
  \begin{alertblock}{Interpretación}
    \scriptsize
    $S^2=10$ es nuestra mejor estimación insesgada de la varianza poblacional $\sigma^2$ a partir de esta única muestra de 5 observaciones.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 2: Operativo (1/2)}
  \footnotesize
  \begin{block}{Problema 5.1.4 --- Derivación de $\Var(\bar{X})=\sigma^2/n$ (Enunciado)}
    Sean $X_1, \ldots, X_n$ independientes con la misma varianza $\sigma^2$. Partiendo de $\Var(\sum_i X_i) = \sum_i \Var(X_i)$ para variables independientes, derive $\Var(\bar{X}) = \sigma^2/n$.
  \end{block}
  ...
  \begin{alertblock}{Estrategia}
    \scriptsize
    Escriba $\bar{X}=\frac{1}{n}\sum X_i$ y use $\Var(aY)=a^2\Var(Y)$ junto con la aditividad de la varianza bajo independencia.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 2: Operativo (2/2)}
  \scriptsize
  Aplicamos linealidad y aditividad: \pause
  ...
  \begin{alertblock}{Interpretación}
    \scriptsize
    El factor $1/n^2$ (de sacar la constante fuera del cuadrado en $\Var(aY)$) es lo que hace que la varianza de la media \emph{decrezca} con $n$, a diferencia de la varianza de la suma, que crece con $n$.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 3: Analítico (1/2)}
  \footnotesize
  \begin{block}{Problema 5.1.7 --- Insesgadez de $S^2$ (Enunciado)}
    Demuestre que $E(S^2) = \sigma^2$ para $X_1, \ldots, X_n$ iid con media $\mu$ y varianza $\sigma^2$. Justifique explícitamente por qué $\sum_{i=1}^n(X_i-\mu) = n(\bar{X}-\mu)$.
  \end{block}
  ...
  \begin{alertblock}{Estrategia}
    \scriptsize
    Escriba $(X_i-\bar{X}) = (X_i-\mu)-(\bar{X}-\mu)$, expanda el cuadrado, sume sobre $i$ y cancele el término cruzado usando la identidad indicada.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 3: Analítico (2/2)}
  \scriptsize
  Expandimos y tomamos esperanza: \pause
  ...
  \begin{alertblock}{Interpretación}
    \scriptsize
    La identidad $\sum(X_i-\mu)=n(\bar X-\mu)$ se sigue directamente de la definición $\bar X = \frac{1}{n}\sum X_i$: es la clave que permite cancelar el término cruzado y aislar el factor $(n-1)$.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 4: Desafiante (1/2)}
  \footnotesize
  \begin{block}{Problema 5.1.10 --- Corrección por Población Finita (Enunciado)}
    Una población finita de $N=200$ cuentas tiene varianza $\sigma^2=900$. Se extrae una muestra de $n=20$ cuentas \emph{sin reemplazo}. Calcule $\Var(\bar{X})$ con la fórmula ingenua y con la FPC, y explique por qué la FPC siempre reduce la varianza.
  \end{block}
  ...
  \begin{alertblock}{Estrategia}
    \scriptsize
    Compare $\sigma^2/n$ contra $(\sigma^2/n)\cdot(N-n)/(N-1)$, y analice el signo de $(N-n)/(N-1) - 1$.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 4: Desafiante (2/2)}
  \scriptsize
  Comparamos ambas fórmulas: \pause

  \vspace{0.05cm}
  \begin{block}{Cálculo}
    \scriptsize
    \begin{align*}
      \Var(\bar{X})_{\text{ingenua}} = \frac{900}{20} = 45, \qquad
  ...
  \vspace{0.05cm}
  \pause
  \begin{alertblock}{¿Por qué la FPC siempre reduce la varianza?}
    \scriptsize
    El factor $(N-n)/(N-1)$ es estrictamente menor que 1 siempre que $n>1$ y $N$ sea finito (pues $N-n<N-1$). Cuando $N\to\infty$, el factor $\to 1$ y ambas fórmulas coinciden: la corrección solo importa cuando se muestrea una fracción no despreciable de una población finita.
  \end{alertblock}
\end{frame}
