...
\begin{frame}{Laboratorio en Python: Varianza y Corrección de Bessel}
  ...
  \\ Script: \texttt{\footnotesize presentaciones/code/01\_estadistica\_descriptiva/01.03\_medidas\_dispersion.py}
...
% Slide 9B: Laboratorio en Python (Outliers e IQR - Código)
\begin{frame}{Laboratorio en Python: Robustez de IQR vs Desviación $S$}
  Experimento inyectando una latencia anómala extrema ($450 \text{ ms}$) en una muestra estable.
  \\ Script: \texttt{\footnotesize presentaciones/code/01\_estadistica\_descriptiva/01.03\_medidas\_dispersion.py}
...
\begin{frame}{Ejercicios del Curso — Nivel 1: Fundamental}
  \begin{block}{Problema 1.4.1 (Sencillo / Directo)}
    Encontrar el rango y la desviación estándar muestral ($S$, con corrección de Bessel) del siguiente arreglo de datos no agrupados:
    \begin{align}
      X = \{12, 6, 7, 3, 15, 10, 18, 5\}
    \end{align}
    \vspace{0.2em}
    \emph{Guía paso a paso:} Calcule la media $\bar{X}$, luego las diferencias $(X_i - \bar{X})^2$, sume y divida entre $n-1 = 7$.
  \end{block}
\end{frame}

% Slide 11: Ejercicios del Curso - Nivel 2
\begin{frame}{Ejercicios del Curso — Nivel 2: Operativo}
  \begin{block}{Problema 1.4.9 (Intermedio / De Desarrollo)}
    Dos grupos de estudiantes tienen las siguientes características muestrales:
    \begin{itemize}
      \item \textbf{Grupo A:} $n_1 = 30$ estudiantes, $\bar{X}_1 = 75$ puntos, $S_1 = 8$ puntos.
      \item \textbf{Grupo B:} $n_2 = 20$ estudiantes, $\bar{X}_2 = 75$ puntos, $S_2 = 12$ puntos.
    \end{itemize}
    \vspace{0.4em}
    Encuentre la desviación estándar del grupo combinado de $n_1 + n_2 = 50$ estudiantes utilizando la relación combinada de suma de cuadrados.
  \end{block}
\end{frame}

% Slide 12: Ejercicios del Curso - Nivel 3
\begin{frame}{Ejercicios del Curso — Nivel 3: Analítico}
  \begin{block}{Problema 1.4.4 (Avanzado / De Conexión)}
    Demostrar rigurosamente y paso a paso por expansión algebraica que para cualquier conjunto de observaciones, la fórmula computacional abreviada para la desviación estándar es:
    \begin{align}
      S = \sqrt{\dfrac{\sum X_i^2 - n\bar{X}^2}{n - 1}} = \sqrt{\dfrac{1}{n-1} \left[ \sum X_i^2 - \dfrac{\left(\sum X_i\right)^2}{n} \right]}
    \end{align}
    \emph{Nota:} Esta expresión requiere una sola pasada por los datos en algoritmos de procesamiento continuo.
  \end{block}
\end{frame}
...
\begin{frame}{Ejercicios del Curso — Nivel 4: Desafiante}
  \vspace{-0.2em}
  \small
  \begin{block}{Problema 1.4.10 (Experto / Comparación Relativa)}
    El coeficiente de variación se define como $CV = \frac{S}{\bar{X}} \times 100\%$. Compare e interprete la variabilidad relativa en las siguientes dos situaciones prácticas de Ciencia de Datos:
    \begin{enumerate}
      \item \textbf{Salarios corporativos:} Empresa A tiene $\bar{X}_A = \$45,000$ con $S_A = \$8,000$; Empresa B tiene $\bar{X}_B = \$38,000$ con $S_B = \$6,500$. ¿Qué nómina es proporcionalmente más dispersa?
      \item \textbf{Biometría humana:} Estatura media $170 \text{ cm}$ con $S = 8 \text{ cm}$; peso medio $70 \text{ kg}$ con $S = 10 \text{ kg}$. ¿Qué variable presenta mayor heterogeneidad intrínseca?
    \end{enumerate}
  \end{block}
\end{frame}
