...
\begin{frame}{Ejercicio en Clase --- Nivel 1: Fundamental (1/2)}
  \footnotesize
  \begin{block}{Problema (Enunciado, Problema 3.9.1)}
    Se lanza un dado regular 120 veces, obteniendo frecuencias observadas $\{18,22,20,19,23,18\}$. Formule $H_0/H_a$, calcule el estadístico $\chi^2$ y los grados de libertad, y decida al $\alpha=0.05$ (valor crítico $\chi^2_{0.05,5}=11.07$).
  \end{block}

  \vspace{0.15cm}
  \pause
  \begin{alertblock}{Planteamiento}
    Bajo uniformidad, cada cara tiene $E_i=120/6=20$.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 1: Fundamental (2/2)}
  \scriptsize
  Calculamos el estadístico sumando las 6 contribuciones: \pause
  ...
  \begin{alertblock}{Interpretación}
    Como $1.10<11.07$, \textbf{no se rechaza $H_0$}: las variaciones observadas son atribuibles al azar del muestreo; el dado se comporta como justo.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 2: Operativo (1/2)}
  \footnotesize
  \begin{block}{Problema (Enunciado, Problema 3.9.2)}
    Una encuesta a $n=200$ consumidores sobre preferencia de color de empaque arroja $\{55,48,42,35,20\}$ para 5 opciones. Pruebe la uniformidad al $\alpha=0.05$ e identifique qué colores generan la mayor divergencia.
  \end{block}
  ...
  \begin{alertblock}{Estrategia}
    \scriptsize
    Bajo uniformidad, $E_i=200/5=40$ para cada color; compare con $\chi^2_{0.05,4}=9.488$.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 2: Operativo (2/2)}
  \scriptsize
  Calculamos el estadístico desglosado por color: \pause
  ...
  \begin{alertblock}{Interpretación}
    \scriptsize
    Como $17.95>9.488$, \textbf{se rechaza $H_0$}. El color ``Otro'' aporta más de la mitad del estadístico (preferencia muy inferior a lo esperado); ``Rojo'' es el favorito indiscutible del mercado.
  \end{alertblock}
\end{frame}
