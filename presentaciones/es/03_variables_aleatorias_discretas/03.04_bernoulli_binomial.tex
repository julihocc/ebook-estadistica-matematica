...
\begin{frame}{Ejercicio en Clase — Nivel 4: Desafiante (2/2)}
  ...
  \begin{alertblock}{2. Caso Especial Bimodal ($m = (n+1)p \in \mathbb{Z}^+$)}
    \fontsize{7pt}{8.5pt}\selectfont
    Si el producto $(n+1)p$ es exactamente un número entero $m$, entonces en el punto $k = m$ el cociente es igual a la unidad ($R(m) = 1 \implies P(X=m) = P(X=m-1)$). La distribución posee exactamente \textbf{dos modas adyacentes} con idéntica masa máxima:
