...
\begin{frame}{Ejercicio en Clase --- Nivel 1: Fundamental (1/2)}
  \footnotesize
  \begin{block}{Problema 3.7.1 --- Llamadas a un Call Center (Enunciado)}
  ...
  \begin{alertblock}{Planteamiento del Modelo}
    El número de llamadas en un intervalo fijo sigue una distribución de Poisson con parámetro $\lambda = 3$:
    $$P(X = k) = \dfrac{e^{-3} \cdot 3^{k}}{k!}, \quad k \in \set{0, 1, 2, \dots}$$
    Calcule explícitamente $P(X = 0)$, $P(X = 1)$, $P(X = 2)$ y $P(X \ge 4)$.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 1: Fundamental (2/2)}
  \scriptsize
  Evaluamos las probabilidades puntuales con la fórmula de Poisson y usamos el complemento para la cola derecha: \pause

  \vspace{0.04cm}
  \begin{block}{Cálculo de Probabilidades Puntuales}
    \begin{itemize}\itemsep=0.04cm
      \item \textbf{Cero llamadas ($X=0$):}
      $P(X=0) = e^{-3} \cdot \dfrac{3^{0}}{0!} = e^{-3} \approx 0.04979$ \pause
      \item \textbf{Una llamada ($X=1$):}
      $P(X=1) = e^{-3} \cdot \dfrac{3^{1}}{1!} = 3e^{-3} \approx 0.14936$ \pause
      \item \textbf{Dos llamadas ($X=2$):}
  ...
  \vspace{0.04cm}
  \pause
  \begin{alertblock}{Cola Derecha por Complemento}
    \scriptsize
    $P(X \ge 4) = 1 - P(X \le 3) = 1 - 13 e^{-3} \approx 1 - 0.6472 = \mathbf{0.3528}$ \quad (35.28\% de probabilidad)
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 2: Operativo (1/2)}
  \footnotesize
  \begin{block}{Problema 3.7.4 --- Ley de Eventos Raros en Ensayo Clínico (Enunciado)}
  ...
  \vspace{0.2cm}
  \pause
  \begin{alertblock}{Comparación Binomial vs. Poisson}
    Compare la probabilidad exacta de observar exactamente $k = 2$ reacciones adversas bajo el modelo binomial $Y \sim \text{Bin}(2000, 0.001)$ con la aproximación de Poisson $P(X = 2) = \dfrac{e^{-2} \cdot 2^{2}}{2!}$. Justifique teóricamente por qué la aproximación es excelente.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 2: Operativo (2/2)}
  \footnotesize
  Evaluamos ambas probabilidades y calculamos el error absoluto: \pause

  \vspace{0.1cm}
  \begin{block}{Cálculo Bajo el Modelo Binomial Exacto}
    $$P(Y = 2) = \binom{2000}{2} (0.001)^{2} (0.999)^{1998} \approx 1\,999\,000 \cdot 10^{-6} \cdot 0.13534 \approx \mathbf{0.27056}$$
  \end{block}

  \vspace{0.1cm}
  \pause
  \begin{block}{Aproximación de Poisson con $\lambda = 2$}
    $$P(X = 2) = \dfrac{e^{-2} \cdot 2^{2}}{2!} = 2 e^{-2} \approx 2 \cdot 0.13534 \approx \mathbf{0.27067}$$
  \end{block}

  \vspace{0.1cm}
  \pause
  \begin{alertblock}{Justificación Teórica}
    El error absoluto es $\abs{0.27056 - 0.27067} \approx 0.00011 < 5 \times 10^{-4}$. La aproximación es excelente porque se cumple la regla operativa $n = 2000 \ge 50$, $p = 0.001 \le 0.05$ y $np = 2 \le 5$.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 3: Analítico (1/2)}
  \footnotesize
  \begin{block}{Problema 3.7.7 --- Deducción Analítica de Momentos (Enunciado)}
    Sea $X \sim \text{Pois}(\lambda)$ una variable aleatoria con parámetro $\lambda > 0$. Demuestre analíticamente que $\E[X] = \lambda$ y $\E[X^{2}] = \lambda^{2} + \lambda$ a partir de la identidad de la serie exponencial $e^{\lambda} = \sum_{k=0}^{\infty} \lambda^{k}/k!$ y derivación término a término.
  \end{block}

  \vspace{0.1cm}
  \pause
  \begin{alertblock}{Estrategia de Deducción}
    La derivación respecto a $\lambda$ de la identidad $e^{\lambda} = \sum \lambda^{k}/k!$ produce $e^{\lambda} = \sum k \lambda^{k-1}/k!$. Multiplicar por $\lambda e^{-\lambda}$ lleva a $\E[X] = \lambda$. La derivada segunda produce $\E[X(X-1)] = \lambda^{2}$, de donde $\E[X^{2}] = \lambda^{2} + \lambda$.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 3: Analítico (2/2)}
  \scriptsize
  Derivamos dos veces la identidad de la serie exponencial para obtener los momentos de Poisson: \pause

  \vspace{0.05cm}
  \begin{block}{Derivada Primera: Esperanza Matemática}
    \scriptsize
    Partimos de $e^{\lambda} = \sum_{k=0}^{\infty} \lambda^{k}/k!$, derivamos: $e^{\lambda} = \sum_{k=1}^{\infty} k \lambda^{k-1}/k!$. Multiplicamos por $\lambda e^{-\lambda}$:
    $$\E[X] = \sum_{k=0}^{\infty} k \cdot \dfrac{e^{-\lambda} \lambda^{k}}{k!} = \lambda e^{-\lambda} \sum_{k=1}^{\infty} \dfrac{\lambda^{k-1}}{(k-1)!} = \lambda e^{-\lambda} \cdot e^{\lambda} = \lambda$$
  \end{block}

  \vspace{0.05cm}
  \pause
  \begin{alertblock}{Conclusión por Equidispersión}
    \scriptsize
    Análogamente, $\E[X(X-1)] = \lambda^{2}$, de donde $\E[X^{2}] = \E[X(X-1)] + \E[X] = \lambda^{2} + \lambda$. Por König-Huygens, $\Var(X) = \E[X^{2}] - (\E[X])^{2} = (\lambda^{2} + \lambda) - \lambda^{2} = \lambda \quad \blacksquare$
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 4: Desafiante (1/2)}
  \footnotesize
  \begin{block}{Problema 3.7.9 --- Distribución Condicional Binomial (Enunciado)}
    Sean $X \sim \text{Pois}(\lambda_{1})$ y $Y \sim \text{Pois}(\lambda_{2})$ dos variables aleatorias independientes que representan el número de eventos atendidos por dos servidores web en un mismo intervalo. Demuestre analíticamente que la distribución condicional de $X$ dado $(X + Y = n)$ es $\text{Bin}(n, p)$ con $p = \lambda_{1}/(\lambda_{1}+\lambda_{2})$.
  \end{block}

  \vspace{0.15cm}
  \pause
  \begin{alertblock}{Aplicación Numérica}
    Con $\lambda_{1} = 3$ y $\lambda_{2} = 5$, si en una hora se atendieron en total $n = 8$ solicitudes entre ambos servidores, calcule la probabilidad de que el servidor $1$ haya atendido exactamente $k = 3$ de ellas.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 4: Desafiante (2/2)}
  \scriptsize
  Por probabilidad condicional, independencia, y el resultado de aditividad (Problema 3.7.8): \pause

  \vspace{0.05cm}
  \begin{block}{Cálculo del Cociente}
    \scriptsize
    $P(X=k \mid X+Y=n) = \dfrac{P(X=k) P(Y=n-k)}{P(X+Y=n)} = \dfrac{\dfrac{e^{-\lambda_{1}}\lambda_{1}^{k}}{k!} \cdot \dfrac{e^{-\lambda_{2}}\lambda_{2}^{n-k}}{(n-k)!}}{\dfrac{e^{-(\lambda_{1}+\lambda_{2})}(\lambda_{1}+\lambda_{2})^{n}}{n!}} = \binom{n}{k} p^{k}(1-p)^{n-k}$
  \end{block}
  ...
  \begin{alertblock}{Aplicación Numérica}
    \scriptsize
    Con $p = 3/8 = 0.375$ y $n = 8$, $k = 3$:
    $$P(X=3 \mid X+Y=8) = \binom{8}{3} \left(\dfrac{3}{8}\right)^{3} \left(\dfrac{5}{8}\right)^{5} = 56 \cdot \dfrac{27}{512} \cdot \dfrac{3125}{32768} \approx 56 \cdot 0.00503 \approx \mathbf{0.2815}$$
    Por lo tanto, dado que se atendieron en total $8$ solicitudes, la probabilidad de que el servidor $1$ haya atendido exactamente $3$ es $\approx 28.15\%$.
  \end{alertblock}
\end{frame}
