...
% Slide 4: MLE y Sobredispersión
\begin{frame}{Estimación por MLE y Detección de Sobredispersión}
  \vspace{-0.15cm}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{MLE de Poisson}
        \footnotesize
        Para datos de conteo $x_{1}, \dots, x_{n}$ con $X_{i} \sim \text{Pois}(\lambda)$:
        $$L(\lambda) = \prod_{i=1}^{n} \dfrac{e^{-\lambda}\lambda^{x_{i}}}{x_{i}!}$$
        $\ell'(\lambda) = -n + \sum x_{i}/\lambda = 0 \implies \hat{\lambda} = \bar{x}$.
      \end{block}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{alertblock}{Índice de Dispersión $D = s^{2}/\bar{x}$}
        \footnotesize
        \begin{itemize}\itemsep=0.04cm
          \item $D \approx 1$: equidispersión; Poisson adecuada.
          \item $D > 1.5$: sobredispersión fuerte; usar Binomial Negativa.
          \item $D < 0.8$: sub-dispersión; usar Conway-Maxwell-Poisson.
        \end{itemize}
      \end{alertblock}
    \end{column}
  \end{columns}
...
% Slide 5: Binomial Negativa como Alternativa
\begin{frame}{Binomial Negativa: la Alternativa Natural ante Sobredispersión}
  \vspace{-0.15cm}
  \begin{block}{PMF de la Binomial Negativa}
    \footnotesize
    La distribución Binomial Negativa introduce un parámetro adicional $r > 0$ para modelar variabilidad extra:
  ...
  \begin{alertblock}{Interpretación Jerárquica}
    ...
    \begin{itemize}\itemsep=0.03cm
      ...
      \item Cuando $r \to \infty$, la Binomial Negativa colapsa a la distribución Poisson.
    \end{itemize}
  \end{alertblock}
\end{frame}

% Slide 6: Criterios de Información
