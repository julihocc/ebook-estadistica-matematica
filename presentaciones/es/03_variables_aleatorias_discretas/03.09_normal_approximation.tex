...
\begin{frame}{Ejercicio en Clase --- Nivel 1: Fundamental (1/2)}
  \footnotesize
  \begin{block}{Problema 3.9.1 --- Distribución Normal de IQ (Enunciado)}
    Una variable aleatoria continua $X$ tiene distribución Normal con media $\mu = 100$ y desviación estándar $\sigma = 15$. Especifique formalmente la PDF $f(x)$ y calcule las probabilidades $P(X \le 115)$ y $P(X \ge 85)$ usando la estandarización al puntaje $Z$ y la CDF de la Normal estándar.
  \end{block}
  ...
  \begin{alertblock}{Planteamiento del Modelo}
    \begin{itemize}\itemsep=0.04cm
      \item $X \sim N(100, 15^{2})$ con PDF $f(x) = \dfrac{1}{15\sqrt{2\pi}} \exp\left(-\dfrac{(x-100)^{2}}{450}\right)$.
      \item Estandarización: $Z = (X - 100)/15 \sim N(0, 1)$.
      \item Use \texttt{scipy.stats.norm.cdf} para evaluar las probabilidades.
    \end{itemize}
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 1: Fundamental (2/2)}
  \scriptsize
  Estandarizamos $Z = (X - 100)/15$ y usamos la simetría de la Normal estándar: \pause

  \vspace{0.05cm}
  \begin{block}{Cálculo de Probabilidades}
    \begin{itemize}\itemsep=0.05cm
      \item \textbf{Cola superior:} $P(X \le 115) = P\left(Z \le \dfrac{115-100}{15}\right) = P(Z \le 1) = \Phi(1) \approx 0.8413$ \pause
      \item \textbf{Cola inferior:} $P(X \ge 85) = P\left(Z \ge \dfrac{85-100}{15}\right) = P(Z \ge -1) = 1 - \Phi(-1) = \Phi(1) \approx 0.8413$
    \end{itemize}
  \end{block}

  \vspace{0.05cm}
  \pause
  \begin{alertblock}{Simetría de la Normal Estándar}
    \scriptsize
    Por la simetría de la campana, $P(X \le 115) = P(X \ge 85) \approx 0.8413$ ($84.13\%$). El intervalo $[85, 115]$ contiene exactamente $\pm 1$ desviación estándar respecto a la media, capturando aproximadamente el $68\%$ de la masa central; cada cola externa vale $\approx 16\%$.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 2: Operativo (1/2)}
  \footnotesize
  \begin{block}{Problema 3.9.4 --- Aproximación Binomial-Normal en Examen Estandarizado (Enunciado)}
    En un examen estandarizado con $n = 200$ preguntas de opción múltiple con 4 opciones cada una, suponga que un estudiante responde al azar con $p = 0.25$ de probabilidad de acierto. Sea $X$ el número de aciertos. Use la aproximación de la Binomial por la Normal con corrección de continuidad de Yates para calcular $P(X \ge 60)$.
  \end{block}

  \vspace{0.15cm}
  \pause
  \begin{alertblock}{Comparación con Valor Exacto}
    \scriptsize
    Calcule la aproximación Normal y compárela con el valor exacto de la Binomial. Las reglas operativas son $np = 50 \ge 5$ y $n(1-p) = 150 \ge 5$, garantizando la validez de la aproximación.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 2: Operativo (2/2)}
  \footnotesize
  Con $\mu = np = 50$ y $\sigma = \sqrt{np(1-p)} = \sqrt{37.5} \approx 6.124$: \pause

  \vspace{0.05cm}
  \begin{block}{Aproximación con Corrección de Yates}
    \scriptsize
    $P(X \ge 60) \approx P(Y \ge 59.5)$ donde $Y \sim N(50, 6.124^{2})$:
  ...
  \begin{alertblock}{Comparación con Valor Exacto}
    \scriptsize
    El valor exacto de la Binomial es $P(X \ge 60) = 1 - \text{stats.binom.cdf}(59, 200, 0.25) \approx 0.0625$. El error absoluto es $\abs{0.0604 - 0.0625} \approx 0.0021$, confirmando la precisión de la aproximación con corrección de Yates.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 3: Analítico (1/2)}
  \footnotesize
  \begin{block}{Problema 3.9.7 --- Deducción del Teorema de De Moivre-Laplace (Enunciado)}
    Sea $X \sim \text{Bin}(n, p)$ con $\mu = np$ y $\sigma^{2} = np(1-p)$. Demuestre que la variable estandarizada $Z_{n} = (X - np)/\sqrt{np(1-p)}$ converge en distribución a la Normal estándar cuando $n \to \infty$.
  \end{block}
  ...
  \begin{alertblock}{Estrategia de Deducción}
    \scriptsize
    Use la MGF de la Binomial $M_{X}(t) = (1 - p + p e^{t})^{n}$ y derive la MGF de $Z_{n}$. Aplique la expansión de Taylor $\log(1 + x) = x - x^{2}/2 + O(x^{3})$ para tomar el límite cuando $n \to \infty$ y obtener $M_{Z}(t) = e^{t^{2}/2}$, la MGF de $N(0, 1)$.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 3: Analítico (2/2)}
  \scriptsize
  Derivamos la MGF de $Z_{n}$ y tomamos el límite asintótico: \pause

  \vspace{0.05cm}
  \begin{block}{MGF de $Z_{n}$ y Límite Asintótico}
    \scriptsize
    $M_{Z_{n}}(t) = e^{-npt/\sigma_{n}} (1 - p + p e^{t/\sigma_{n}})^{n}$, donde $\sigma_{n} = \sqrt{np(1-p)}$.
  \end{block}

  \vspace{0.05cm}
  \pause
  \begin{alertblock}{Expansión de Taylor del Logaritmo}
    \scriptsize
    ...
    \end{align*}
    Multiplicando por $n$ y centrando: $n \log(\cdots) - npt/\sigma_{n} \to t^{2}/2$. Por lo tanto:
...
\begin{frame}{Ejercicio en Clase --- Nivel 4: Desafiante (1/2)}
  \footnotesize
  \begin{block}{Problema 3.9.9 --- TLC para Suma de Poisson (Enunciado)}
    Sea $X_{1}, \dots, X_{n}$ i.i.d. Poisson($\lambda$). Por TLC, $S_{n} = \sum X_{i}$ converge a $N(n\lambda, n\lambda)$. Para $n = 100$ y $\lambda = 2$:
    \begin{itemize}\itemsep=0.04cm
      \item Calcule $P(S_{100} \le 220)$ usando la Poisson exacta.
      \item Calcule la aproximación Normal con corrección de continuidad.
    \end{itemize}
  \end{block}
  ...
  \begin{alertblock}{Planteamiento del Modelo}
    \scriptsize
    $S_{100} \sim \text{Pois}(200)$ con $\mu = 200$, $\sigma = \sqrt{200} \approx 14.142$. Use la regla operativa $n\lambda = 200 \ge 30$ para la aproximación Normal.
  \end{alertblock}
\end{frame}
...
\begin{frame}{Ejercicio en Clase --- Nivel 4: Desafiante (2/2)}
  \scriptsize
  Comparamos la Poisson exacta con la aproximación Normal con corrección de Yates: \pause

  \vspace{0.05cm}
  \begin{block}{Valor Exacto Poisson}
    \scriptsize
  ...
  \vspace{0.05cm}
  \pause
  \begin{alertblock}{Aproximación Normal con Yates}
    \scriptsize
    Con $Y \sim N(200, 200)$ y $\sigma = \sqrt{200} \approx 14.142$:
    \begin{align*}
      P(S_{100} \le 220) &\approx P\left(Y \le 220.5\right) = \Phi\left(\dfrac{220.5 - 200}{14.142}\right) = \Phi(1.450) \approx 0.9264
    \end{align*}
    El error absoluto es $\abs{0.9247 - 0.9264} \approx 0.0017$, confirmando la precisión del TLC para la suma de 100 variables Poisson(2). \qedhere
  \end{alertblock}
\end{frame}
