...
### Añadido / Corregido
- Actualizados `presentaciones/README.md`, `presentaciones/ROADMAP.md` y `presentaciones/ESPECIFICACIONES_Y_REQUERIMIENTOS.md` para que la regla vigente sea Bloom (Recordar → Comprender → Aplicar → Analizar → Evaluar → Crear), con etiquetas `prob:<7-hex>` y soluciones enlazadas mediante `\begin{solproblema}[prob:<tag>]`.
- Marcadas como históricas y no normativas las referencias a 10 problemas `3-3-2-2`; corregida la referencia errónea a una Unidad 8 para el capítulo 09, que es contenido complementario.
- Corregidos estados de catálogo y referencias a bundles retirados; el ROADMAP ya no contiene nombres vivos de `distribuciones_especiales(p).tex` ni de los demás bundles reemplazados.
- Registrado el estado real: 54 mazos ES, 54 mazos EN, 54 PDF ES, 54 PDF EN, 53 scripts Python y 60 pares de cuadernos `(p).tex` ES/EN.

### Verificación
- Confirmado que los cambios se limitan a documentación; no se modificaron `presentaciones/es/`, `presentaciones/en/` ni `presentaciones/code/`.
- Confirmado que el backlog técnico queda reducido al solapamiento de captions ANOVA y a la diferencia de orden de la distribución exponencial.
- Enlaces relativos y reglas históricas verificados antes del commit; no se hará `git push`.
...
- **Sin `git commit` en ningún punto de la migración registrada el 2026-07-22** — el diff completo de los 21 archivos queda staged para revisión del usuario, siguiendo la misma convención que la sesión de migración anterior.
...
- **Reorganización de cuadernos de problemas y contenido de práctica en los mazos — decisión de alcance, no ejecutada aún.** Durante la Fase G (mapeo estricto 1 mazo = 1 `\section`), el usuario pidió replantear dos convenciones establecidas desde las Fases A/B y codificadas como estándar oficial en `presentaciones/ESPECIFICACIONES_Y_REQUERIMIENTOS.md` (Regla de Oro 1/Bloque IV y Regla de Oro 4):
  ...
  5. **Alcance retroactivo:** aplica a los capítulos ya reestructurados al 2026-07-22 (Cap. 3-5, vía Fases C/E/G1-G3) y hacia adelante (Cap. 6-9, Fase F).
...
- **Divergencia estructural ES/EN del libro maestro — Fase C (Nivel 1) completa:** el usuario reportó que, pese a las Fases A/B, el desalineamiento entre las notas ES y EN seguía presente y que las tablas de contenido tampoco coincidían con los slides. Una nueva auditoría confirmó que las Fases A/B nunca compararon la estructura real de `\section`/`\subsection` (solo paridad de problemas y frames de teoría en slides), y encontró divergencias genuinas de tabla de contenidos:
  ...
  - **Hallazgo adicional descubierto durante la etapa documentada el 2026-07-22, no reportado originalmente por el usuario:** los números de problema impresos en el PDF no coinciden con lo que citan los mazos de slides (ej. mazo cita "Problema 3.5.2", el PDF real imprime "3.10.11"), causado por el uso de `\subsection*` con asterisco (que no avanza el contador real de LaTeX) para organizar sub-temas dentro de varios archivos `(p).tex`. Confirmado contra el `.aux` compilado. Afecta 7 pares de archivos ES+EN en 5 capítulos (3, 4, 5, 6, 8); no afecta Caps. 1, 2, 7, 9.
