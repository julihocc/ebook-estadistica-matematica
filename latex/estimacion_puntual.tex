...
En esta sección estudiaremos los dos métodos de estimación puntual más fundamentales de la teoría estadística moderna: el \emph{Método de Momentos} (MoM) y el \emph{Método de Máxima Verosimilitud} (MLE). Asimismo, analizaremos su papel en el diseño y entrenamiento de modelos en ciencia de datos y aprendizaje automático.
...
\begin{ejemplo}
	\textbf{Un caso delicado: cuando el primer momento no identifica al parámetro.}
	Sea $X \sim U(-\theta, \theta)$ con $\theta > 0$ desconocido. Por la simetría de la distribución, el primer momento poblacional es idénticamente nulo para cualquier valor de $\theta$:
	\begin{align}
		\mu_1 = E(X) = 0, \quad \text{para todo } \theta > 0.
	\end{align}
	Igualar $\mu_1 = m_1 = \bar{X}$ no permite despejar $\theta$: el primer momento simplemente no contiene información sobre el parámetro. Debemos recurrir al primer momento \emph{no trivial}, en este caso el segundo momento respecto al origen. Como $\text{Var}(X) = \theta^2/3$ y $\mu_1 = 0$:
	\begin{align}
		\mu_2 = \text{Var}(X) + \mu_1^2 = \frac{\theta^2}{3}.
	\end{align}
	Igualando con el segundo momento muestral $m_2 = \frac{1}{n}\sum X_i^2$:
	\begin{align}
		\frac{\theta^2}{3} = m_2 \implies \hat{\theta}_{MoM} = \sqrt{3 m_2} = \sqrt{\frac{3}{n}\sum_{i=1}^n X_i^2}.
	\end{align}
\end{ejemplo}

\begin{observacion}[Propiedades y limitaciones del Método de Momentos]
	El MoM tiene ventajas y desventajas claras frente al MLE:
	\begin{itemize}
		\item \textbf{Consistencia:} por la Ley de los Grandes Números, $m_k \xrightarrow{P} \mu_k(\theta)$ para cada $k$; bajo condiciones de regularidad (continuidad de la función que relaciona momentos con parámetros), esto implica que $\hat{\theta}_{MoM} \xrightarrow{P} \theta$.
		\item \textbf{Simplicidad computacional:} a menudo produce fórmulas cerradas (como en el ejemplo Gamma anterior) incluso cuando el MLE requiere optimización numérica iterativa.
		\item \textbf{Menor eficiencia:} en general, $\hat{\theta}_{MoM}$ no alcanza la Cota de Cramér-Rao y tiene mayor varianza asintótica que $\hat{\theta}_{MLE}$; por ello, el MoM se usa frecuentemente solo como valor inicial para algoritmos iterativos de MLE (ver aplicaciones en ciencia de datos, más abajo).
		\item \textbf{Estimaciones fuera del espacio paramétrico:} a diferencia del MLE (que por construcción siempre produce un valor admisible), el MoM puede en ocasiones arrojar estimaciones fuera de $\Theta$ (por ejemplo, una varianza estimada negativa en modelos con restricciones no lineales entre momentos y parámetros).
	\end{itemize}
\end{observacion}
...
\subsubsection{La Función de Score y Normalidad Asintótica}

La cantidad que se iguala a cero en la condición de primer orden tiene nombre e importancia propios: es la \emph{función de score}.

\begin{definicion}[Función de score]
	Para una sola observación $X$ con densidad $f(x\mid\theta)$, la \emph{función de score} es
	\begin{align}
		U(\theta; X) = \frac{\partial}{\partial\theta}\ln f(X\mid\theta).
	\end{align}
	Para una muestra $X_1,\ldots,X_n$ iid, el \emph{score total} es $U_n(\theta) = \sum_{i=1}^n U(\theta; X_i) = \dfrac{d}{d\theta}\ell(\theta)$. El estimador de máxima verosimilitud es, por definición, la solución de la \emph{ecuación de score} $U_n(\hat\theta_{MLE}) = 0$.
\end{definicion}

\begin{propiedad}[Propiedades del score]
	Bajo condiciones de regularidad que permiten intercambiar derivación e integración:
	\begin{enumerate}
		\item \textbf{Media cero:} $E_\theta[U(\theta;X)] = 0$ para el valor verdadero de $\theta$.
		\item \textbf{Identidad de la información:} $\text{Var}_\theta[U(\theta;X)] = I(\theta)$, es decir, la varianza del score de una observación es exactamente la Información de Fisher.
	\end{enumerate}
\end{propiedad}

\begin{proof}[Bosquejo de la demostración de (1)]
	Como $\int f(x\mid\theta)\,dx=1$ para todo $\theta$, derivando ambos lados respecto a $\theta$ e intercambiando derivada e integral:
	\begin{align}
...
\begin{teorema}[Normalidad asintótica del MLE]
	 \label{eq:6.3.1}
	...
	\end{align}
	es decir, para $n$ grande, $\hat\theta_{MLE} \overset{\text{aprox.}}{\sim} N\!\left(\theta,\, \dfrac{1}{nI(\theta)}\right)$.
\end{teorema}

\begin{observacion}[Conexión con la Cota de Cramér-Rao]
	Este resultado formaliza la \emph{eficiencia asintótica} del MLE mencionada en la Sección 06.01: la varianza asintótica del MLE, $1/(nI(\theta))$, coincide exactamente con la Cota Inferior de Cramér-Rao. Así, aunque el MLE no sea insesgado ni óptimo para $n$ finito (como vimos con $\hat\sigma^2_{MLE}$), se vuelve asintóticamente óptimo: ningún estimador consistente puede hacerlo mejor en el límite.
\end{observacion}
