...
\subsubsection{Experimentos aleatorios}
...
\begin{ejemplo}
	\label{exmp:2.3.7}
	Si lanzamos una moneda dos veces seguidas un posible espacio muestral esta dado en el ejemplo \ref{exmp:2.3.3},  mientras que otro esta dado por
	\begin{align}
		\set{(0,0), (0,1), (1,0),(1,1)}.
	\end{align}
\end{ejemplo}
...
\begin{ejemplo}
	\label{exmp:2.3.8}
	Si lanzamos una moneda dos veces, el evento de que obtengamos \emph{exactamente} un reverso  es un subconjunto del espacio muestral mostrado en la figura \ref{fig:2.3.2}.
\end{ejemplo}
...
\subsubsection{Operaciones entre eventos}
...
\subsubsection{Enfoques de la probabilidad}
...
\begin{observacion}
	Ambos enfoques tienen sus inconvenientes:
	\begin{enumerate}
...
\subsubsection{Los Axiomas de la probabilidad}
...
\subsubsection{Algunos teoremas importantes en probabilidad}
...
\begin{proof}
	Como $A_1 \subset A_2$, podemos escribir $A_2 = A_1 \cup (A_2 - A_1)$ donde $A_1$ y $A_2 - A_1$ son disjuntos.
	...
	Como $P(A_2 - A_1) \geq 0$ (axioma 1), se sigue que $P(A_1) \leq P(A_2)$.
	
	Despejando: $P(A_2 - A_1) = P(A_2) - P(A_1)$.
\end{proof}
...
\begin{proof}
	La desigualdad $P(A) \geq 0$ se sigue directamente del axioma 1.
	
	Para demostrar $P(A) \leq 1$, observamos que $A \subset S$, por lo que aplicando el teorema \ref{thm:2.3.1}:
	\begin{align}
		P(A) \leq P(S) = 1
	\end{align}
	por el axioma 2.
...
\begin{proof}
	Como $\emptyset \subset S$ y $\emptyset \cap S = \emptyset$, podemos escribir $S = S \cup \emptyset$ como unión disjunta.
...
\begin{teorema}
	\label{thm:2.3.4} La probabilidad de un evento complementario está dada por
	\begin{align}
		\label{eq:2.3.6}
		P(A')=1-P(A)
	\end{align}
...
\begin{proof}
	Como $A$ y $A'$ son disjuntos y $A \cup A' = S$, por el axioma 3:
	\begin{align}
		P(S) &= P(A \cup A') = P(A) + P(A')
	\end{align}
	
	Como $P(S) = 1$ (axioma 2), despejando obtenemos:
	\begin{align}
		P(A') = 1 - P(A)
	\end{align}
\end{proof}
...
\begin{teorema}
	\label{thm:2.3.5} Si $A=A_{1}\sqcup...\sqcup A_{N}$ es la unión disjunta de eventos mutuamente excluyentes entonces
	\begin{align}
		\label{eq:2.3.7}
		P(A)=P(A_{1})+...+P(A_{N}).
	\end{align}
...
\subsubsection{Asignación de probabilidades}

Si un espacio muestral consiste en una cantidad \emph{finita} de posibles resultados $a_{1},...,a_{N},$ entonces por el teorema \ref{thm:2.3.5},
\begin{align}
	\label{eq:2.3.13}
	P(A_{1})+...+P(A_{n})=1
\end{align}
...
Se sigue que uno puede escoger de manera arbitraria cualesquiera números no negativos como probabilidades de estos eventos simples, siempre que se satisfaga \eqref{eq:2.3.13}.
