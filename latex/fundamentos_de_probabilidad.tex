...
\begin{ejemplo}
	\label{exmp:2.2.7}
	Si lanzamos una moneda dos veces seguidas un posible espacio muestral esta dado en el ejemplo \ref{exmp:2.2.3},  mientras que otro esta dado por
	\begin{align}
		\set{(0,0), (0,1), (1,0),(1,1)}.
	\end{align}
\end{ejemplo}
...
\begin{ejemplo}
	\label{exmp:2.2.8}
	Si lanzamos una moneda dos veces, el evento de que obtengamos \emph{exactamente} un reverso  es un subconjunto del espacio muestral mostrado en la figura \ref{fig:2.2.2}.
\end{ejemplo}
...
\begin{proof}
	...
	Para demostrar $P(A) \leq 1$, observamos que $A \subset S$, por lo que aplicando el teorema \ref{thm:2.2.1}:
	\begin{align}
		P(A) \leq P(S) = 1
	\end{align}
	por el axioma 2.
...
\begin{teorema}
	\label{thm:2.2.4} La probabilidad de un evento complementario está dada por
	\begin{align}
		\label{eq:2.2.6}
		P(A')=1-P(A)
	\end{align}
...
\begin{teorema}
	\label{thm:2.2.5} Si $A=A_{1}\sqcup...\sqcup A_{N}$ es la unión disjunta de eventos mutuamente excluyentes entonces
	\begin{align}
		\label{eq:2.2.7}
		P(A)=P(A_{1})+...+P(A_{N}).
	\end{align}
...
Si un espacio muestral consiste en una cantidad \emph{finita} de posibles resultados $a_{1},...,a_{N},$ entonces por el teorema \ref{thm:2.2.5},
\begin{align}
	\label{eq:2.2.13}
	P(A_{1})+...+P(A_{n})=1
\end{align}
...
Se sigue que uno puede escoger de manera arbitraria cualesquiera números no negativos como probabilidades de estos eventos simples, siempre que se satisfaga \eqref{eq:2.2.13}.
