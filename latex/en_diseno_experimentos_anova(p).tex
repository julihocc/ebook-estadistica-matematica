...
\begin{problema}
	...
	Given that the omnibus ANOVA in Problem \ref{prob:anova_un_factor_ml} was highly significant, apply **Tukey's HSD post-hoc procedure** with a family-wise significance level $\alpha = 0.05$ to simultaneously compare all six possible pairs of algorithms and precisely identify which models differ from one another in mean performance.
\end{problema}
...
\begin{problema}
	...
	Therefore, a **Randomized Complete Block Design (RCBD)** is designed using the physical generations of $b = 4$ distinct corporate servers as the blocking factor (Server 1: Latest Gen, Server 2: Mid Gen, Server 3: Old Gen, Server 4: Legacy virtualized Gen). Each engine is evaluated exactly once within each server, yielding the data shown in Table \ref{tab:dbca_datos}.
	\begin{table}[h!]
		\centering
...
\begin{problema}
	...
	\begin{enumerate}
		\item Calculate the Least Significant Difference (**Fisher's LSD**) at $\alpha = 0.05$.
		\item Calculate the critical difference margin using the **Bonferroni Correction** method to maintain a maximum family-wise error rate of $\alpha = 0.05$.
...
\begin{problema}
	...
	\begin{enumerate}
		...
		\begin{align}
			...
			E[\text{CME}] &= \sigma^2.
		\end{align}
		\item In a Completely Randomized Design (CRD) that ignores blocks but uses the exact same $N = kb$ experimental units, let the estimated residual variance be denoted by $S_{\text{CRD}}^2$. Deduce from the above expectations the formula for the **Relative Efficiency ($\text{ER}$)** of the RCBD compared to the CRD:
		\begin{align}
...
\begin{solucion}
	...
	\end{align}
	Consequently, **any pair of algorithms whose sample means differ in absolute value by more than $5.811\%$ will be considered statistically different** under strict family-wise error control at the $5\%$ level.
	...
	\begin{enumerate}
		\item \textbf{XGBoost vs SVM:} $|\bar{Y}_{\text{XGB}} - \bar{Y}_{\text{SVM}}| = |88.6 - 79.8| = 8.80\% > 5.811\%$ $\implies$ **Significant ($*$)**.
		...
		\item \textbf{Neural Networks vs SVM:} $|\bar{Y}_{\text{DNN}} - \bar{Y}_{\text{SVM}}| = |86.4 - 79.8| = 6.60\% > 5.811\%$ $\implies$ **Significant ($*$)**.
	...
	\end{enumerate}
	**Technical post-hoc conclusion:** Tukey's HSD procedure reveals and isolates precisely the source of ANOVA significance: **both XGBoost ($88.6\%$) and Deep Neural Networks ($86.4\%$) significantly outperform SVM ($79.8\%$)** in $F_1$-score. Between XGBoost, Neural Networks, and Random Forests, there are no statistically discernible differences given the study's sample size.
\end{solucion}
...
\begin{solucion}
	\begin{enumerate}
		...
		Because our sample statistic exceeds the boundary by an overwhelming margin ($16.370 \gg 4.637$), **we reject the null hypothesis of equal strengths at the 1\% significance level**. Chemical formulation type decisively alters the mechanical tensile strength of the polymer.
	\end{enumerate}
\end{solucion}
...
\begin{solucion}
	...
	\begin{align}
		F_{0.05, \, 2, \, 6} = 5.143.
	\end{align}
	Since the treatment statistic $F_{\text{TR}} = 52.667 \gg 5.143$, **we reject $H_{0,\text{TR}}$**. There is an extremely significant difference in processing times across the three SQL engines (observing clearly from treatment averages that **Engine B** with $\bar{Y}_{2.} = 14.50$ s is the fastest of the three).
	...
	**Methodological conclusion on design:** Incorporating server generations as blocks was a **brilliantly effective and indispensable** methodological decision. Blocking absorbed and isolated $382.0$ units of pure infrastructure variance ($\text{SCB}$). If we had not blocked the design and instead analyzed the data using standard one-way ANOVA, those $382$ variance points would have been added directly to the residual error ($\text{SCE}_{\text{one-way}} = 382 + 3 = 385$), yielding a much larger residual variance $\text{CME}_{\text{one-way}} = \frac{385}{12-3} = 42.77$.
	...
	\end{align}
	Blocking **completely rescued the experiment from a massive false negative (Type II error)** by purging hardware noise.
\end{solucion}
...
\begin{solucion}
	...
	The Bonferroni threshold ($7.540$ ms) is **nearly $50\%$ more stringent and wide** than the Fisher LSD threshold ($5.046$ ms).
	\begin{itemize}
...
\begin{solucion}
	\begin{enumerate}
		...
		Since $F = 23.953 \gg 3.634$, **we reject $H_0$**. Average CPU bandwidth consumption differs drastically across the virtualization environments.
		
		\item \textbf{Calculation of $R^2$ and adjusted $R_{\text{adjusted}}^2$ coefficients:}
		The **Coefficient of Determination ($R^2$ or Eta-squared $\eta^2$)** measures the total fraction of variability explained by the main factor:
		\begin{align}
		...
		The **adjusted $R^2$** penalizes the degrees of freedom consumed by treatments relative to the total:
		\begin{align}
...
\begin{solucion}
	The analyst's assertion is **dangerously incomplete under the analyzed scenario**. It is a mathematical fact that ANOVA possesses structural robustness against mild-to-moderate homoscedasticity deviations when sample sizes are balanced ($n_1 = n_2 = n_3$). However, this robustness **does not withstand extreme disparities** across group variances.
	
	In our case, the variance of Treatment 3 ($S_3^2 = 8.9$) is more than **seven times larger** than the variance of Treatment 1 ($S_1^2 = 1.2$, ratio $8.9 / 1.2 = 7.41$). This severe heterogeneity indicates that Treatment 3 operates with vastly higher systemic instability or contains severe outliers.
	
	\textbf{Verification procedure and rigorous methodological action:}
	\begin{enumerate}
		\item **Qualitative and quantitative residual audit:** Construct a Studentized residual plot and inspect whether the dispersion exhibits a funnel or trumpet pattern (the classic hallmark of mean-dependent non-constant variance).
		\item **Formal Levene or Brown-Forsythe test:** Subject the sample to Levene's test ($\alpha = 0.05$). Given a variance ratio of $7.4:1$, Levene's test will decisively reject the null hypothesis $H_0: \sigma_1^2 = \sigma_2^2 = \sigma_3^2$.
		\item **Appropriate analytical alternative:** Given the confirmed homoscedasticity failure, the analyst must discard the conventional $F$ statistic and employ **Welch's ANOVA (or Welch's $F$-test)**, which inversely weights group mean squares by individual variances $S_i^2/n_i$ and adjusts denominator degrees of freedom, or alternatively apply a logarithmic transformation ($\log Y_{ij}$) if data are positive and right-skewed.
	\end{enumerate}
\end{solucion}
...
\begin{solucion}
	...
	\end{align}
	A value of $f = 0.44$ represents a **medium-to-large** effect size magnitude on Cohen's standardized scale ($f=0.10$ small, $0.25$ medium, $0.40$ large).
	...
	\textbf{Experimental design conclusion:} Rounding up to the next integer to strictly guarantee achieving the desired target power, we must assign a sample size of **$n = 14$ replicates per backend language**, resulting in a total experiment size of **$N = k \times n = 4 \times 14 = 56$ audited API requests**.
\end{solucion}
	...
	\begin{enumerate}
		...
		**William G. Cochran's Theorem** formally establishes in linear algebra that: if a standard multivariate normal vector $\mathbf{Z} \sim N(\mathbf{0}, \mathbf{I}_N)$ satisfies the quadratic form partitioning $\mathbf{Z}^T \mathbf{Z} = \sum_{m=1}^M \mathbf{Z}^T \mathbf{A}_m \mathbf{Z}$, where matrices $\mathbf{A}_m$ are symmetric with ranks $r_m = \text{rank}(\mathbf{A}_m)$, then **the quadratic forms $\mathbf{Z}^T \mathbf{A}_m \mathbf{Z}$ are mutually and unconditionally independent Chi-square random variables with $r_m$ degrees of freedom, if and only if the sum of their ranks equals the total space dimension: $\sum_{m=1}^M r_m = N$**.
		...
		\begin{align}
			(k - 1) + (N - k) = N - 1 = \nu_T.
		\end{align}
		Because the dimensional partitioning condition of Cochran's Theorem is satisfied with rigorous exactness, the projection matrices of $\text{SCTR}$ and $\text{SCE}$ are **idempotent and mutually orthogonal ($\mathbf{A}_{\text{TR}} \mathbf{A}_E = \mathbf{0}$)**. Therefore, the variables $\text{CMTR}$ and $\text{CME}$ are stochastically and functionally independent.
...
\begin{solucion}
	\begin{enumerate}
		...
		\end{align}
		The **Relative Efficiency ($\text{ER}$)** is defined as the ratio of experimental error variances ($S_{\text{CRD}}^2 / S_{\text{RCBD}}^2 = S_{\text{CRD}}^2 / \text{CME}$):
		\begin{align}
		...
		\end{align}
		**Theoretical conclusion:** The Randomized Complete Block Design **yields a strict, tangible gain in estimation efficiency ($\text{ER} > 1$) whenever the Block Mean Square exceeds the Error Mean Square ($F_{\text{Block}} > 1$)**. If the blocking factor was irrelevant or poorly chosen such that $\text{CMB} < \text{CME}$, blocking acts as an analytical drag that unnecessarily wastes error degrees of freedom without absorbing infrastructure variance.
	\end{enumerate}
\end{solucion}
