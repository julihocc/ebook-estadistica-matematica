...
To incorporate qualitative categorical attributes accurately into a regression architecture, we construct indicator variables, commonly referred to as \textbf{dummy variables}.
...
If a customer resides neither in Tier 1 ($X_{t1}=0$) nor in Tier 2 ($X_{t2}=0$), then logically and necessarily that customer resides in Tier 3. Therefore, an explicit dummy variable is omitted for one reference category to prevent exact linear dependence across columns (known as the \textbf{dummy variable trap}).
...
Frequently, the quantitative target variable does not maintain a straight linear relationship with one or more continuous explanatory variables; instead, the underlying physical or economic phenomenon exhibits a \textbf{non-linear functional relationship}.
...
\end{align}
visual inspection of the scatter plot reveals unmistakable downward curvature. The points clearly follow a curved, parabolic trajectory much better described by a \textbf{quadratic polynomial model}:
\begin{align}
