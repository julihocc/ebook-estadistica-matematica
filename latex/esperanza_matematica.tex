...
siempre y cuando dicha \emph{integral} converja.
 ...
 \begin{ejemplo}
  \label{exmp:2.9.1}
  Supongamos que un juego se juega con un dado único que se suponen justos. En este juego, un jugador gana \$20 si un sale un $2$; \$40 con un $4$; \$60 con un $6$; y no gana ni pierde con cualquier otra cara. Encuentre la suma esperada de dinero que ganaría.
 \end{ejemplo}

\begin{solucion}
	Sea $X$ la ganancia del jugador. La función de probabilidad es:
	\begin{align}
		f(x) = \begin{cases}
			\frac{1}{6} & x = 20 \\
			\frac{1}{6} & x = 40 \\
	...
	En promedio, el jugador gana \$20 por juego.
\end{solucion}
...
{Esperanza e independencia}
 \begin{teorema}
  \label{thm:2.9.2} Si $X,Y$ son \emph{variables aleatorias independientes}, entonces
  \begin{align}
   \label{eq:2.9.8}
...
\subsection{Varianza y Desviación Estándar}
 ...
 Ya vimos que la esperanza matemática de una variable aleatoria $X$ es una medida de tendencia central y que generaliza a la \emph{media aritmética $\mu$.}
...
\end{figure}
 ...
 \begin{ejemplo} %falta escribir esto en la libreta
  \label{exmp:2.9.4}
  Encuentre la varianza y la desviación estándar de la variable aleatoria del ejemplo \ref{exmp:2.9.2}.
 \end{ejemplo}
