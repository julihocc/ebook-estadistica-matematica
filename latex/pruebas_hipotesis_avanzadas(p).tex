...
\begin{problema}
	...
	\begin{enumerate}
		...
		\item Suponga que debido a una sobrecarga en la red, la latencia media real de la población aumentó silenciosamente a $\mu_a = 108$ ms. Calcule la probabilidad exacta de cometer un **Error Tipo II ($\beta$)** —no detectar la sobrecarga— y determine el valor exacto de la **Potencia de la prueba ($1-\beta$)**.
		\item Si el director de ingeniería exige que la prueba mantenga un riesgo $\alpha = 0.05$ pero sea capaz de detectar una sobrecarga real de $\mu_a = 108$ ms con una **Potencia estadística del $90\%$** ($1-\beta = 0.90 \implies Z_\beta = 1.282$), deduzca y calcule exactamente qué tamaño de muestra $n$ debe procesar el sistema en cada auditoría.
	\end{enumerate}
\end{problema}
...
\begin{problema}
	...
	\textbf{Deducción de la Prueba de Razón de Verosimilitudes (*Likelihood Ratio Test*, LRT) para varianzas e invocación del Teorema de Wilks:}
...
\begin{solucion}
	...
	\begin{align}
		-Z_{0.02} = -2.054.
	\end{align}
	Como el estadístico observado $Z = -3.881$ cae en la zona de rechazo ($Z < -2.054$, con un p-valor $< 0.0001$), **rechazamos $H_0$**. Demostramos al nivel de significancia del $2\%$ que el nuevo Modelo de IA tiene una tasa de detección superior al Filtro Clásico.
\end{solucion}
...
\begin{solucion}
	...
	Dado que $\chi^2 = 56.147 \gg 13.277$ (p-valor $< 0.00001$), **rechazamos $H_0$**. La distribución de preferencias de monetización no es idéntica; la Generación X prefiere fuertemente el pago único ($70$ vs $41.25$ esperados) mientras que la Gen Z se decanta en forma abrumadora por el modelo Freemium ($90$ vs $63.33$).
\end{solucion}
...
\begin{solucion}
	\begin{enumerate}
		...
		Puesto que $t = 3.121 > 2.101$, **rechazamos $H_0$**. Concluimos al 95\% de confianza que el Algoritmo 2 consume un tiempo de ejecución medio significativamente menor que el Algoritmo 1.
	\end{enumerate}
\end{solucion}
...
\begin{solucion}
	...
	Como $\chi^2 = 18.5714 > 7.815$, **rechazamos $H_0$**. Las tasas de cumplimiento ético difieren entre las cuatro empresas.
	...
	\begin{itemize}
		...
		Al ser $0.180 > 0.1719$, **la diferencia A vs B es estadísticamente significativa**.
	...
	\end{itemize}
	**Conclusión:** Las parejas con diferencias reales comprobadas son **A vs. B** ($82\%$ vs $64\%$), **A vs. D** ($82\%$ vs $55\%$) y **C vs. D** ($75\%$ vs $55\%$).
\end{solucion}
...
\begin{solucion}
	\begin{enumerate}
		...
		\end{align}
		**Regla operativa:** Si en una auditoría diaria el promedio muestral de transacciones supera los $104.935$ ms ($\bar{X} > 104.935$), el sistema rechaza $H_0$ y activa una alerta de sobrecarga en la red.
		
		\item \textbf{Cálculo de la probabilidad de Error Tipo II ($\beta$) y Potencia ($1-\beta$) con $\mu_a = 108$ ms:}
		El **Error Tipo II ($\beta$)** se comete al aceptar erróneamente $H_0$ ($\bar{X} \le \bar{X}_c$) cuando en el mundo real la hipótesis alternativa es verdadera ($\mu = 108$ ms):
		\begin{align}
		...
		\end{align}
		La **Potencia estadística ($1-\beta$)** —la probabilidad real de detectar con éxito la sobrecarga— resulta por diferencia directa:
		\begin{align}
...
\begin{solucion}
	\begin{enumerate}
		...
		**Contraste conceptual con $F$ de Snedecor:** Mientras que la prueba de Razón de Verosimilitudes (LRT) invocada vía Wilks es un estadístico **asintótico** válido solo en muestras grandes y extremadamente sensible al supuesto normal, la **prueba $F$ de Snedecor ($F = s_1^2 / s_2^2$) es la prueba exacta para muestras finitas normales** uniformemente más potente (UMP insesgada), siendo por tanto la opción preferida por excelencia para diseños experimentales de tamaño pequeño o mediano en docimasia analítica.
	\end{enumerate}
\end{solucion}
...
\begin{solucion}
	\begin{enumerate}
		...
		\end{align}
		Esta es la identidad exacta: **la densidad en el espacio de p-valores en cualquier punto $u$ es idénticamente el Cociente de Verosimilitud evaluado en el cuantil docimásica correspondiente ($t = S_0^{-1}(u)$)**.
