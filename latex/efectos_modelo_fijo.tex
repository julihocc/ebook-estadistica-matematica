...
Para comprender la lógica del estadístico de prueba del ANOVA, debemos analizar la \textbf{esperanza matemática de los cuadrados medios}. Sea $\nu_E = N - k$ los grados de libertad del error:
\begin{align}
	E[\text{CME}] &= \sigma^2, \\
...
Formulamos las hipótesis estadísticas del contraste global como:
\begin{align}
	H_0&: \tau_1 = \tau_2 = \ldots = \tau_k = 0 \quad \Longleftrightarrow \quad \mu_1 = \mu_2 = \ldots = \mu_k, \\
	H_a&: \tau_i \neq 0 \text{ para al menos un } i \quad \Longleftrightarrow \quad \mu_i \neq \mu_j \text{ para al menos un par } (i,j).
\end{align}
...
\begin{teorema}[Estadístico de prueba $F$ en ANOVA]
	...
	\begin{align}
		F = \frac{\text{CMTR}}{\text{CME}} = \frac{\text{SCTR} / (k - 1)}{\text{SCE} / \nu_E}
	\end{align}
	sigue una distribución $F$ de Fisher-Snedecor central con $\nu_1 = k - 1$ grados de libertad en el numerador y $\nu_2 = \nu_E = N - k$ grados de libertad en el denominador.

	Si la hipótesis alternativa $H_a$ es verdadera, el término cuadrático $\sum n_i \tau_i^2 / (k-1)$ es estrictamente positivo y el estadístico sigue una distribución $F$ \textbf{no central}, con parámetro de no-centralidad $\lambda = \frac{1}{\sigma^2}\sum_{i=1}^k n_i \tau_i^2$, provocando que $F$ tome valores sistemáticamente mayores.
...
\begin{teorema}[Criterio LSD de Fisher]
	...
	\begin{align}
		|\bar{Y}_{i.} - \bar{Y}_{j.}| > \text{LSD} = t_{\alpha/2, \, \nu_E} \sqrt{\text{CME} \left( \frac{1}{n_i} + \frac{1}{n_j} \right)},
	\end{align}
	donde $t_{\alpha/2, \nu_E}$ es el cuantil superior de la distribución $t$ de Student con los grados de libertad del error del ANOVA ($\nu_E$).
\end{teorema}
...
\begin{teorema}[Criterio HSD de Tukey para tamaños de muestra iguales]
	...
	\begin{align}
		|\bar{Y}_{i.} - \bar{Y}_{j.}| > \text{HSD} = q_{\alpha, \, k, \, \nu_E} \sqrt{\frac{\text{CME}}{n}},
		\label{eq:tukey_hsd}
	\end{align}
	donde $q_{\alpha, k, \nu_E}$ es el valor crítico en las tablas del rango estandarizado para $k$ tratamientos y $\nu_E$ grados de libertad en el error.
\end{teorema}
...
\end{align}
Dos medias se declaran significativamente diferentes si $|t_{ij}| > t_{\alpha^*/2, \nu_E}$, o bien si su diferencia supera el margen crítico $t_{\alpha/(2m), \nu_E} \sqrt{\text{CME}(\frac{1}{n_i}+\frac{1}{n_j})}$.
