...
\begin{enumerate}
	\item Suponga que el valor del parámetro asumido en la hipótesis nula es $A_0$. 
	\item Tomemos
...
El investigador conoce la desviación estándar del parámetro de su experiencia pasada.



Un buen ejemplo de esto es el caso del tiempo de entrega de una pizza.  En este caso \eqref{eq:3.6.1} seguirá una distribución normal y los valores normalizados se conocerán como \emph{valores Z}.
...
\begin{solucion}
	Buscamos el valor $t$ tal que el área a la derecha sea $0.05$. Esto significa que el área a la izquierda es $1 - 0.05 = 0.95$.
	
	Usando la función de percentil (ppf) de la distribución $t$ con $\nu=9$ grados de libertad:
	\begin{align}
		t_{0.05, 9} = t_{0.95} \approx 1.833
	\end{align}
	
	Por simetría de la distribución $t$, el valor que deja un área de $0.05$ a la izquierda es $-1.833$.
	
	El área total sin sombrear (entre $-1.833$ y $1.833$) es:
	\begin{align}
