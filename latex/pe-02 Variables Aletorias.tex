...
en todos aquellos puntos en que $f(x)$ sea continua.  Es decir, la derivada de la función de distribución es la función de densidad.
 ...
 \begin{ejemplo}
  \label{sol:0.2.5} Una variable aleatoria $X$ tiene función de densidad
  \begin{align}
...
tal que el volumen bajo la superficie es igual a $1.$
	...
	\begin{align}
	 ...
	 f(x,y)  \label{eq:0.2.23} \\
	  	 P(X\leq x)=F_{X}(x)=\int_{-\infty}^{x}\int_{-\infty}^{\infty} f(u,v)dvdu \label{eq:0.2.24} \\
	 	 P(Y\leq y)=F_{Y}(y)=\int_{-\infty}^{\infty}\int_{-\infty}^{y} f(u,v)dvdu \label{eq:0.2.25}
	\end{align}
