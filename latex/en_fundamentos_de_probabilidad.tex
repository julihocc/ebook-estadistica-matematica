...
\begin{example}
	\label{exmp:2.2.7}
	If we toss a coin twice consecutively, one possible sample space is given in Example \ref{exmp:2.2.3}, while another is given by
	\begin{align}
		\set{(0,0), (0,1), (1,0),(1,1)}.
	\end{align}
\end{example}
...
\begin{example}
	\label{exmp:2.2.8}
	If we toss a coin twice, the event that we obtain \emph{exactly} one tail is a subset of the sample space shown in Figure \ref{fig:2.2.2}.
\end{example}
...
\begin{proof}
	...
	To prove $P(A) \leq 1$, observe that $A \subset S$, so applying Theorem \ref{thm:2.2.1}:
	\begin{align}
		P(A) \leq P(S) = 1
	\end{align}
	by Axiom 2.
...
\begin{theorem}
	\label{thm:2.2.4} The probability of the complementary event is given by
	\begin{align}
		\label{eq:2.2.6}
		P(A')=1-P(A)
	\end{align}
...
\begin{theorem}
	\label{thm:2.2.5} If $A=A_{1}\sqcup...\sqcup A_{N}$ is the disjoint union of mutually exclusive events, then
	\begin{align}
		\label{eq:2.2.7}
		P(A)=P(A_{1})+...+P(A_{N}).
	\end{align}
...
If a sample space consists of a \emph{finite} number of possible outcomes $a_{1},...,a_{N},$ then by Theorem \ref{thm:2.2.5},
\begin{align}
	\label{eq:2.2.13}
	P(A_{1})+...+P(A_{n})=1
\end{align}
...
It follows that one can arbitrarily choose any non-negative numbers as probabilities of these simple events, provided that \eqref{eq:2.2.13} is satisfied.
