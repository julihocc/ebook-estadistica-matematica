...
\begin{solproblema}[prob:f43c638]
	\begin{enumerate}
		...
		\end{align*}
		Similarly, $\E[X^2] = 8 = 2/\lambda^2$, so $\Var(X) = 8-4=4=1/\lambda^2$.
...
\begin{solproblema}[prob:7b147c4]
	\begin{enumerate}
		...
		\item For $\sigma/\sqrt{n}<0.01$ with $\sigma=0.1$: $\sqrt{n}>10$, so $n>100$, i.e. at least $n=101$ measurements are needed. With $n=100$ the standard error is exactly $0.01$ (the boundary, which does not satisfy the strict inequality); with $n=101$, $\sigma/\sqrt{n}\approx0.00995<0.01$ (less than 1\% of the true value).
	\end{enumerate}
\end{solproblema}
